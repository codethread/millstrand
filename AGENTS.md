...
**Landing a finished branch** — coordinator-only: `strand workflow show land`, then use generic workflow verbs and `strand help land` for the policy boundaries. `.skein/.land-merge-lock.acquire` is the coordination-owned cross-process mutex file; leave it alone. An active merge-lock conflict means another coordinator is merging to main, so wait rather than starting competing landing work. Required checks are strict here, so a merge gate failing with "N of N required status checks are expected" usually means the PR is `BEHIND` main (`gh pr view <n> --json mergeStateStatus`) rather than that a check went red: rebase onto `origin/main`, push, re-establish green CI at the new HEAD, then clear the gate stamp.
