...
**Landing a finished branch** — coordinator-only: `strand workflow show land`, then use generic workflow verbs and `strand help land` for the policy boundaries. `.skein/.land-merge-lock.acquire` is the coordination-owned cross-process mutex file; leave it alone. When another coordinator holds the lock, join the merge train with `strand land await <land-run-id>` rather than retrying the approval or starting competing landing work: awaiting reserves your turn from sign-off onward, turns are served first-in first-out, and timing out keeps your place. A granted turn still means one rebase onto `origin/main` and one CI run — read `landed-since` for what to absorb. An ungranted return names the runs ahead with their stage and last update; only break the lock once you have established that a stalled head cannot resume. Required checks are strict here, so a merge gate failing with "N of N required status checks are expected" usually means the PR is `BEHIND` main (`gh pr view <n> --json mergeStateStatus`) rather than that a check went red: rebase onto `origin/main`, push, re-establish green CI at the new HEAD, then clear the gate stamp.
