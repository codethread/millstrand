...
**Starting multi-step work** — `mill strand prime`: the planning/tracking workflow. Feature work starts with the registered `intake` workflow; read `strand workflow show intake` for its parameter contract before starting the run. Large module changes pour the registered story workflow; invocation and discipline come from `strand prime workflow` and `strand about workflow`.
