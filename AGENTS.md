...
- PRs go through the `land` workflow (`strand workflow show land`). Use ordinary `workflow choose` / `complete` / `await` verbs. Sign-off queues automatic landing; `strand prime merge-queue` explains FIFO status and explicit withdrawal.
