...
Mill owns the complete `restart.json` record: `state`, `transition_id`, optional generation identities, optional non-blank `previous_weaver_id`, `updated_at`, `old_generation_stopped`, the probe result (`success`, stage, probe/source workspaces, completed stages, diagnostics, and log), and the failure (`stage`, message, and optional log or exit evidence). Mill validates this schema and its state relationships before each write and after every read. The peer client validates the same closed shape when a record is present, including unknown, missing, wrong-type, and contradictory fields. Malformed records are refused loudly; an identity mismatch by itself means the record does not describe that peer's transition and remains an ordinary transport failure. Startup reclaims stale runtime artifacts only with a matching world identity, a proven dead owner, and an inactive socket; live, malformed, or unknown owners remain untouched.
