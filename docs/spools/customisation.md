...
Routine `mill weaver status` and `mill weaver list` use a compact cached projection of this record. They retain lifecycle state, generation and transition identities, and the short failure explanation, but never emit the probe or its diagnostics. The `mill weaver restart --json` result and the retained `restart.json` record retain the complete probe report for explicit diagnosis. A changed local record invalidates the compact projection before the next poll.
