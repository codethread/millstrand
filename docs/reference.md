...
(`ct/policy/config.clj`, the focused modules under `ct/workflows/`, `agents/harnesses.clj`, `ct/agents/reviewers.clj`, `ct/notifications/attention.clj`, and `ct/jobs/nvd_scan.clj`) — is documented in the `init.clj` header itself.

The focused modules under `.millstrand/ct/workflows/` author this repo's static `defworkflow` Vars. The shared `ct/workflows/common.clj` module keeps the weave patterns; `ct/workflows/land.clj`, `workflows/spool_bump.clj`, `ct/workflows/story.clj`, `ct/workflows/explore.clj`, and `ct/workflows/fix.clj` each own a workflow family. Because the definitions are static, a worker reads them with the shipped generic surface: `strand workflow list` for the catalogue, `strand workflow show story` for a definition's param contract, then `start`, `ready`, `next`, `complete`, `choose`, `defer`, and `await` to drive a run.
