...
User-facing Millstrand documentation lives in the source checkout under `docs/`; the canonical user reference is this page, `docs/reference.md`. Two harness-agnostic orientation commands surface this to agents at runtime, with no running weaver required: `mill prime millstrand` resolves the Millstrand source and prints the paths to the docs, the spool index, and the repo coordination guidance, plus how to extend `.millstrand` or `.ms` config; `mill prime strand` prints the strand planning/tracking workflow. In a repo-world bootstrap, `mill init` also seeds a `## Millstrand / strand` section in the repository-root `AGENTS.md`/`CLAUDE.md` that points new agents at these two commands. Each shipped spool's per-fn API reference (`spools/*.api.md`) and each blessed `millstrand.api.*.alpha` namespace's per-fn reference (`docs/api/*.api.md`) is generated from source docstrings and regenerated with `make api-docs` — never hand-edited; see the [spool index](../spools/README.md#doc-triad) for the contract/cookbook/generated-API triad. The [alpha API index](./api/README.md) maps each `millstrand.api.*.alpha` namespace to the concern it owns; the generated pages are reference only, and the behavior contracts remain the root specs (see the [spec index](#spec-index)).

When working in this repository, also read the "Repo coordination workspace (.millstrand)" section of the
root [`AGENTS.md`](../AGENTS.md): the `.millstrand` shared coordination world, its working discipline,
and pointers into the live surface (`strand help`, `strand query list`, `strand pattern list`, and `strand workflow list` for registered ops, queries, patterns, and workflows). The config layout — `.millstrand/init.clj` activation order and its per-concern modules
(`ct/policy/config.clj`, the focused modules under `ct/workflows/`, `workflows/scripts/`, `ct/agents/guide.clj`, `ct/agents/reviewers.clj`, `ct/agents/delegation_contracts.clj`, `ct/notifications/attention.clj`, `ct/jobs/nvd_scan.clj`, and `ct/adapters/help.clj`) — is documented in the `init.clj` header itself.

The focused modules under `.millstrand/ct/workflows/` author this repo's static `defworkflow` Vars. `ct/workflows/support.clj` provides shared shell helpers, and `ct/workflows/common.clj` keeps the weave patterns. `ct/workflows/land.clj`, `ct/workflows/story.clj`, `ct/workflows/explore.clj`, and `ct/workflows/fix.clj` each own a workflow family; `ct/workflows/land_policy.clj` owns the narrow land op. Because the definitions are static, a worker reads them with the shipped generic surface: `strand workflow list` for the catalogue, `strand workflow show story` for a definition's param contract, then `start`, `ready`, `next`, `complete`, `choose`, `defer`, and `await` to drive a run.
...
| `prime` | **Authored** prose orientation | "How do we work here?" — run **before** starting work | `mill prime millstrand`, `mill prime strand`, `strand prime agent` |
