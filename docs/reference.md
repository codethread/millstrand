...
`.skein/workflows.clj` authors this repo's workflow definitions as static `defworkflow` Vars: `land`
with its `land-merge`/`land-abort` continuations, and `story` with its `story-fold`/`story-keep`
continuations. Because they are static, a worker reads them with the shipped generic surface —
`strand workflow list` for the catalogue and `strand workflow show story` for a definition's param
contract — which `.skein/init.clj` activates as its own module beside the engine. That surface also
drives any run: `strand workflow start <id> --workflow story --params '{...}'`, then `ready`,
`complete`, `choose`, `continue`, and `await`.

The `land` op survives beside that generic surface because it adds behavior the engine has no
business knowing: the singleton merge lock and the kanban lane moves. It drives the **landing
workflow** (family `land`), the coordinator-only discipline for taking a finished branch to landed.
It is a single sequential
