...
The focused modules under `.millstrand/workflows/` author this repo's static `defworkflow` Vars. The shared `workflows/common.clj` module keeps the weave patterns; `workflows/land.clj`, `workflows/spool_bump.clj`, `workflows/story.clj`, `workflows/explore.clj`, and `workflows/fix.clj` each own a workflow family. Because the definitions are static, a worker reads them with the shipped generic surface: `strand workflow list` for the catalogue, `strand workflow show story` for a definition's param contract, then `start`, `ready`, `next`, `complete`, `choose`, `defer`, and `await` to drive a run.
