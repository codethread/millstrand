...
A namespace groups related names, like a module or file. `.millstrand/me/policy/config.clj` declares `(ns me.policy.config ...)`, so its top-level definitions live under `me.policy.config`. From elsewhere you can reach one with a qualified symbol such as `me.policy.config/work`, or bring names in unqualified with `require`.
