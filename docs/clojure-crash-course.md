...
A namespace groups related names, like a module or file. `.millstrand/ct/policy/config.clj` declares `(ns ct.policy.config ...)`, so its top-level definitions live under `ct.policy.config`. From elsewhere you can reach one with a qualified symbol such as `ct.policy.config/work`, or bring names in unqualified with `require`.
