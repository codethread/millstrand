...
The Millstrand core is temporarily distributed as SHA-only alpha work. It publishes no marker, and `v1` is forbidden. Published consumers pin the repository and full commit:
...
Use the local form only while developing. Release proof has two stages: `candidate` checks a clean consumer against the candidate checkout, and `published` resolves the fetched Git commit without a local root.
