(ns workflows-fix
  "The light `fix` bug-fix workflow (family \"fix\").

  Less ceremony than devflow, but the doc discipline is structural: after
  implementation, a docs-sync step and machine docs-check gate prove the prose
  gates green before handing off to coordinator land."
  (:require [clojure.spec.alpha :as s]
            [skein.api.format.alpha :as format-alpha]
            [skein.spools.workflow :as workflow]
            [workflows-common :as common]))

(s/def ::fix-params (s/keys :req-un [::common/subject ::common/branch ::common/worktree]
                            :opt-un [::common/card]))
