(ns workflows-story
  "The module-form `story` workflow (family \"story\").

  The forcing function for writing module code: identify changed modules, make
  overall changes, adversarial intent review, then the refactor wave per chunky
  module with a fold-or-keep checkpoint."
  (:require [clojure.spec.alpha :as s]
            [skein.api.format.alpha :as format-alpha]
            [skein.spools.workflow :as workflow]
            [workflows-common :as common]))

(s/def ::module ::common/non-blank-string)
(s/def ::reviewer-harness ::common/non-blank-string)

(s/def ::story-params (s/keys :req-un [::common/feature ::module ::common/worktree]
                              :opt-un [::common/card ::reviewer-harness]))
(s/def ::story-continuation-params (s/keys :req-un [::common/feature ::module ::common/worktree]
                                           :opt-un [::common/card ::reviewer-harness]))
