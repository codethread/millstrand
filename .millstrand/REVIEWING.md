...
A nonempty change for which every available reviewer reports `glob-mismatch` is a valid no-applicable-reviewer outcome. The local `millstrand-review` workflow records that outcome with `AUTOMATIC_REVIEW_NO_APPLICABLE_REVIEWER`; it does not claim a successful review or a no-findings verdict. Empty input, unavailable seats, invalid selectors or results, and failed reviewer runs remain failures and cannot use that outcome.
