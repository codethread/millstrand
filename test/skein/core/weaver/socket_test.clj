...
(defn drain-events!
  ...
  (let [signal (promise)]
    ...
    (events/register-handler! rt :event-drain #{:test/event-drain}
                              'skein.core.weaver.socket-test/event-drain-handler {})
    (try
...
(defn snapshot-probe-op-v1
  ...
  (weaver/replace-op! runtime 'snapshot-probe raw-mutating-standard
                      'skein.core.weaver.socket-test/snapshot-probe-op-v2)
  {:version :v1})
...
(s/def :skein.core.weaver.socket-test.plan/feature plan-non-blank?)
(s/def :skein.core.weaver.socket-test.plan/title plan-non-blank?)
(s/def :skein.core.weaver.socket-test.plan/key plan-non-blank?)
(s/def :skein.core.weaver.socket-test.plan/body plan-non-blank?)
(s/def :skein.core.weaver.socket-test.plan/kind #{"task" "review"})
(s/def :skein.core.weaver.socket-test.plan/hitl boolean?)
(s/def :skein.core.weaver.socket-test.plan/depends_on
  (s/coll-of :skein.core.weaver.socket-test.plan/key :kind vector?))
(s/def :skein.core.weaver.socket-test.plan/max-attempts pos-int?)
(s/def :skein.core.weaver.socket-test.plan/task
  (s/keys :req-un [:skein.core.weaver.socket-test.plan/key :skein.core.weaver.socket-test.plan/title]
          :opt-un [:skein.core.weaver.socket-test.plan/body :skein.core.weaver.socket-test.plan/kind
                   :skein.core.weaver.socket-test.plan/hitl :skein.core.weaver.socket-test.plan/depends_on
                   :skein.core.weaver.socket-test.plan/max-attempts]))
(s/def :skein.core.weaver.socket-test.plan/tasks
  (s/coll-of :skein.core.weaver.socket-test.plan/task :kind vector? :min-count 1))
(s/def :skein.core.weaver.socket-test.plan/input
  (s/and map?
         #(every? #{:feature :title :tasks :body} (keys %))
         (s/keys :req-un [:skein.core.weaver.socket-test.plan/feature :skein.core.weaver.socket-test.plan/title
                          :skein.core.weaver.socket-test.plan/tasks]
                 :opt-un [:skein.core.weaver.socket-test.plan/body])))
(s/def :skein.core.weaver.socket-test.pipeline/input
  (s/and map?
         #(s/valid? :skein.core.weaver.socket-test.plan/tasks (:tasks %))))
...
(deftest json-socket-invoke-dispatch
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'custom raw-mutating-standard 'skein.core.weaver.socket-test/test-op)
      ...
      (testing "context envelope fields ride the invoke arguments"
        (weaver/register-op! rt 'ctx raw-mutating-standard
                             'skein.core.weaver.socket-test/envelope-echo-op)
  ...
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'streamer raw-mutating-unbounded
                           'skein.core.weaver.socket-test/gated-stream-op)
      (let [m (:metadata rt)]
        (with-open [ch (doto (SocketChannel/open StandardProtocolFamily/UNIX)
                         (.connect (UnixDomainSocketAddress/of (:socket-path m))))
                    rdr (BufferedReader. (InputStreamReader. (Channels/newInputStream ch)))
  ...
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'streamer-error raw-mutating-unbounded
                           'skein.core.weaver.socket-test/stream-error-op)
      (let [m (:metadata rt)]
        (with-open [ch (doto (SocketChannel/open StandardProtocolFamily/UNIX)
                         (.connect (UnixDomainSocketAddress/of (:socket-path m))))
                    rdr (BufferedReader. (InputStreamReader. (Channels/newInputStream ch)))
...
(deftest json-socket-invoke-honors-op-deadline
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'slow raw-mutating-standard 'skein.core.weaver.socket-test/slow-op)
...
(deftest json-socket-invoke-payload-hooks-gate-mutating-ops
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'mutate raw-mutating-standard 'skein.core.weaver.socket-test/test-op)
      (weaver/register-op! rt 'reader raw-read-standard 'skein.core.weaver.socket-test/test-op)
      (hooks/register-hook! rt :payload #{:payload/received} 'skein.core.weaver.socket-test/capture-hook {})
      ...
      (let [ctx (first @hook-contexts)]
        ...
        (is (= 'skein.core.weaver.socket-test/capture-hook (:hook/fn ctx)))
      ...
      (testing "subcommand help aliases resolve before mutating hook gating"
        (weaver/register-op! rt 'subbed-mutate
                             ...
                             'skein.core.weaver.socket-test/side-effecting-op)
        (reset! hook-contexts [])
        (reset! op-side-effects [])
  ...
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'paced
                           ...
                           'skein.core.weaver.socket-test/gated-deadline-op)
...
(deftest json-socket-invoke-read-ops-skip-hooks-and-protocol-errors
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'reader raw-read-standard 'skein.core.weaver.socket-test/test-op)
      (hooks/register-hook! rt :payload #{:payload/received} 'skein.core.weaver.socket-test/rejecting-hook {})
  ...
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'mutate raw-mutating-standard
                           'skein.core.weaver.socket-test/side-effecting-op)
      (hooks/register-hook! rt :reject-payload #{:payload/received} 'skein.core.weaver.socket-test/rejecting-hook {})
...
(deftest json-socket-invoke-error-details-are-json-safe
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'boom raw-mutating-standard 'skein.core.weaver.socket-test/throwing-op)
  ...
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'keyword-code raw-mutating-standard
                           'skein.core.weaver.socket-test/keyword-code-op)
  ...
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'non-string-code raw-mutating-standard
                           'skein.core.weaver.socket-test/non-string-code-op)
      (weaver/register-op! rt 'nil-code raw-mutating-standard
                           'skein.core.weaver.socket-test/nil-code-op)
      (weaver/register-op! rt 'opaque-code raw-mutating-standard
                           'skein.core.weaver.socket-test/opaque-code-op)
  ...
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'factory-not-found raw-mutating-standard
                           'skein.core.weaver.socket-test/factory-not-found-op)
  ...
  (with-runtime
    (fn [rt _]
      (weaver/register-op! rt 'factory-canonical-query raw-mutating-standard
                           'skein.core.weaver.socket-test/factory-canonical-query-op)
