...
(defn- contribution-forms
  ...
  [module-key]
  (format
   (str "(let [contribution (get @skein.core.weaver.modules-test/module-contributions %s)]\n"
        "  (case contribution\n"
        "    :skein.core.weaver.modules-test/throw (throw (ex-info \"contribution boom\" {}))\n"
        "    :skein.core.weaver.modules-test/malformed (runtime/collect-module-entry! :queries nil nil)\n"
  ...
  (with-runtime
    (fn [rt _db-file]
      (letfn [(refusal-data [opts]
        ...
        (let [data (refusal-data {:ns 'skein.core.weaver.modules-test
                                  :load :classpath})]
        ...
        (is (nil? (refusal-data {:ns 'skein.core.weaver.modules-test :load :image}))
    ...
    (fn [rt _db-file]
      (let [workspace (get-in rt [:metadata :config-dir])
        ...
        (write-local-spool-module!
         ...
         "(skein.api.runtime.alpha/collect-entry! :queries \"image-live\" [:= [:attr :owner] \"image\"])")
        (is (= :applied
        ...
        (write-local-spool-module!
         ...
         (str "(clojure.spec.alpha/def ::widget map?)\n"
              ...
              (pr-str {:name "bad-glossary"
                       :fn 'skein.core.weaver.modules-test/test-op
                       :stream? false
                       :provenance 'skein.core.weaver.modules-test
    ...
    (fn [rt _db-file]
      (let [workspace (get-in rt [:metadata :config-dir])
        ...
        (write-local-spool-module!
         ...
         "(skein.api.runtime.alpha/collect-entry! :queries \"stamp-q\" [:= [:attr :v] 1])")
...
(deftest lifecycle-callables-must-resolve-to-functions
  (let [callable 'skein.core.weaver.modules-test/bad-lifecycle-callable
        error (try
      ...
      (let [workspace (get-in rt [:metadata :config-dir])
            ...
            (fn [filename form-first?]
              (let [source (str "modules/" filename ".clj")
                    file (io/file workspace source)
                    module-form
                    "(skein.api.runtime.alpha/collect-entry! :queries \"owned\" [:= [:attr :v] 1])\n"]
                (io/make-parents file)
                (spit file
                      (str "(ns " first-ns ")\n"
    ...
    (fn [rt _db-file]
      (let [workspace (get-in rt [:metadata :config-dir])
        ...
        (events/register-handler! rt :refresh-failure #{:refresh/fail}
                                  'skein.core.weaver.modules-test/failing-event {})
        ...
        (events/register-handler! rt :refresh-block #{:refresh/block}
                                  'skein.core.weaver.modules-test/slow-capture-event {})
        ...
        (let [workspace (get-in rt [:metadata :config-dir])
          ...
          (doseq [field [:contribute :reconcile]]
            (let [failure (try
                            (weaver-runtime/declare-module!
                             rt :fresh-legacy
                             {:file source field 'skein.core.weaver.modules-test/module-contribute})
                            nil
                            (catch clojure.lang.ExceptionInfo e e))]
      ...
      (try
        ...
        (spit (io/file workspace "init.clj")
              (str "(skein.core.weaver.runtime/declare-module! "
                   ...
                   ":contribute 'skein.core.weaver.modules-test/module-contribute})\n"))
        (let [failure (try
