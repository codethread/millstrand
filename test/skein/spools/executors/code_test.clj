(ns skein.spools.executors.code-test
  ...
  (:require [clojure.string :as str]
            ...
            [skein.spools.executors.code :as code]
      ...
      (doseq [[index [gate-attrs expected]]
              (map-indexed
               ...
               [[{"code/params" {}} "code/fn"]
                ...
                [(request "skein.spools.executors.code-test/missing" {}) "did not resolve"]
                [{"code/fn" "skein.spools.executors.code-test/return-value"} "code/params"]
                [(request "skein.spools.executors.code-test/return-value" []) "code/params"]
                [(request "skein.spools.executors.code-test/return-value" {} 0)
                 "code/timeout-secs"]])]
  ...
  (with-code
    (fn [rt]
      (workflow/start! "late"
                       (gated-gate
                        "late"
                        (request "skein.spools.executors.code-test/late-value" {}))
                       {})
      ...
      (let [run-id "saturation"
            gates (mapv (fn [index]
                          (workflow/gate
                           ...
                           :attributes
                           (assoc (request
                                   "skein.spools.executors.code-test/wait-for-release"
                                   {:value index})
                                  "test/run-id" run-id)))
                        (range 9))]
        (try
    ...
    (fn [rt]
      ...
      (try
        (workflow/start!
         ...
         (single-gate
          "stubborn"
          (request "skein.spools.executors.code-test/ignore-interrupt-until-release"
                   {:value "late"}
                   1))
         {})
        (test-alpha/await-quiescent! rt)
    ...
    (fn [rt]
      ...
      (let [marker (temp-file)]
        (workflow/start!
         ...
         (single-gate
          "poll"
          (request "skein.spools.executors.code-test/poll-short-subprocesses"
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [pool (:worker-executor
        ...
        (is (= :skein/spools-code (:owner declaration)))
