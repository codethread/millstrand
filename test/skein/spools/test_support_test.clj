(ns skein.spools.test-support-test
...
(deftest temp-dir-creates-fresh-directory
  (let [dir (test-support/temp-dir "skein-test-support")]
    (try
      (is (.isDirectory dir))
      (is (not= (.getCanonicalPath dir)
                (.getCanonicalPath (test-support/temp-dir "skein-test-support"))))
      (finally
        (test-support/delete-tree! dir)))))

(deftest delete-tree-removes-nested-content-and-is-idempotent
  (let [root (test-support/temp-dir "skein-delete-tree")
...
(deftest delete-tree-removes-symlink-without-following-directory-target
  (let [base (test-support/temp-dir "skein-delete-link")
...
(deftest run-git-returns-raw-output-and-describes-failure
  (let [root (test-support/temp-dir "skein-run-git")]
    (try
      (is (= "git-test\n"
