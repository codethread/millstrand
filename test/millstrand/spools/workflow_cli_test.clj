(ns millstrand.spools.workflow-cli-test
...
(defn- activate-cli!
  ...
  [rt]
  (activate-engine! rt)
  (test-support/activate-spool! rt :millstrand/spools-workflow-cli 'millstrand.spools.workflow.cli
...
(defn- register!
  ...
  (doseq [name names]
    (workflow/register-workflow!
     name (symbol "millstrand.spools.workflow-cli-test" (clojure.core/name name)))))
...
(deftest activating-the-workflow-engine-publishes-no-cli-ops
  ...
  (with-runtime
    (fn [rt _]
      ...
      (is (nil? (ns-resolve 'millstrand.spools.workflow 'spool))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [entry (weaver/resolve-op rt 'workflow)]
        ...
        (is (= 'millstrand.spools.workflow.cli (:provenance entry)))
        (is (= 'millstrand.spools.workflow.cli/workflow-op (:fn entry)))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [entry (weaver/resolve-op rt 'workflow)]
        ...
        (is (nil? (ns-resolve 'millstrand.spools.workflow.cli 'spool))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (is (= [{:name "build"
               ...
               :entrypoints ["start" "continue" "call"]
               :definition "millstrand.spools.workflow-cli-test/build"}
              {:name "review"
               :doc "Review a completed implementation."
               :entrypoints ["start" "call"]
               :definition "millstrand.spools.workflow-cli-test/review"}
              {:name "spike"
               :doc "Reduce uncertainty and recommend the next routine."
               :entrypoints ["start" "continue"]
               :definition "millstrand.spools.workflow-cli-test/spike"}]
             (listed))
    ...
    (fn [rt _]
      ...
      (is (= :workflow/definition-invalid
             (reason-of #(workflow/register-workflow!
                          :legacy 'millstrand.spools.workflow-cli-test/legacy-spike)))))))
...
(deftest discovery-fails-loudly-on-a-definition-whose-var-vanished
  ...
  (with-runtime
    (fn [rt _]
      (activate-cli! rt)
      (register! :build)
      (wf-registry/register-definition! rt :ghost 'millstrand.spools.workflow-cli-test/no-such-definition)
      (let [data (ex-data (try (listed) (catch clojure.lang.ExceptionInfo e e)))]
        (is (= :workflow/definition-unresolvable (:reason data)))
        (is (= :ghost (:name data)))
        (is (= 'millstrand.spools.workflow-cli-test/no-such-definition (:definition data)))
    ...
    (fn [rt _]
      ...
      (let [view (shown :spike)]
        ...
        (testing "the param contract is the live spec, its form graph, and the defaults"
          (is (= {:kind "spec"
                  :defaults {:prototype-targets ["compact queue"]}
                  :spec "millstrand.spools.workflow-cli-test/spike-params"
                  :spec-forms [{"spec" "millstrand.spools.workflow-cli-test/spike-params"
                                "relation" "root"
                                "form" (pr-str (s/form ::spike-params))}
                               {"spec" "millstrand.spools.workflow-cli-test/brief"
                                "relation" "keyword-reference"
                                "form" "clojure.core/string?"}
                               {"spec" "millstrand.spools.workflow-cli-test/scope"
                                "relation" "keyword-reference"
                                "form" "clojure.core/string?"}]}
                 (-> (:params view)
...
(deftest executors-projects-declared-request-contracts
  ...
  (with-runtime
    (fn [rt _]
      (activate-cli! rt)
      (test-support/activate-spool! rt :millstrand/spools-shell 'millstrand.spools.executors.shell
                                    :after [:millstrand/spools-workflow])
      (workflow/register-executor! :bare-sym 'millstrand.spools.workflow-cli-test/never-stalled)
      ...
      (let [result (cli/workflow-op {:op/args {:subcommand ["executors"]}})
        ...
        (testing "a declared executor projects its gate-request contract"
          (let [item (by-waiter "shell")
            ...
            (is (= "millstrand.spools.executors.shell/shell-stalled?"
                   (:stall-predicate item)))
            (is (= "millstrand.spools.executors.shell/request" (:spec request)))
            (is (= ["shell/argv"]
        ...
        (testing "a bare-symbol executor lists with no contract"
          (is (= {:waiter "bare-sym"
                  :stall-predicate "millstrand.spools.workflow-cli-test/never-stalled"}
                 (by-waiter "bare-sym"))))
