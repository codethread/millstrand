(ns millstrand.api.events.alpha-test
  ...
  deliberately retained in `millstrand.core.weaver.hooks-events-test` and
...
(def ^:private handler-sym
  ...
  'millstrand.api.events.alpha-test/capture-event)
    ...
    (let [rt (:runtime ctx)]
      ...
      (is (thrown-with-msg? ExceptionInfo #"must resolve to a callable value"
                            (events/register-handler!
                             rt :k #{:strand/added}
                             'millstrand.api.events.alpha-test/not-callable)))
...
(deftest failure-record-spec-pins-the-promised-key-set
  ...
  ;; it back) is locked in `millstrand.core.weaver.hooks-events-test`; this pins the spec against
  ;; that record's documented shape.
  (is (s/valid? ::events/failure-record
                {:handler/key :fails
                 :handler/fn 'millstrand.api.events.alpha-test/failing-event
                 :event/id "evt-1"
  ...
  (t/with-weaver-world [ctx {:storage :sqlite-memory}]
    (let [rt (:runtime ctx)]
      ...
      (let [{:keys [effective shadowed contenders]} (get (events/handler-provenance rt) :h)]
        (is (= :millstrand.owner/repl (:owner effective))
