(ns millstrand.api.spec.alpha-test
...
(declare dotted.pred)
(s/def ::dotted millstrand.api.spec.alpha-test/dotted.pred)
...
(deftest keys-required-vs-optional-and-qualification
  (let [node (spec-alpha/contract ::outer)
    ...
    (is (= "millstrand.api.spec.alpha-test/outer" (get node "spec")))
    (let [qualified (entry-for required "millstrand.api.spec.alpha-test/qualified-key")
          unqualified (entry-for required "nested")]
      (is (true? (get qualified "qualified")))
      (is (false? (get unqualified "qualified")))
      (is (= "millstrand.api.spec.alpha-test/nested" (get unqualified "spec")))
...
(deftest nested-keys-collections-and-scalar-operators
  (let [node (spec-alpha/contract ::outer)
    ...
    (is (= "millstrand.api.spec.alpha-test/plain-str?" (get-in tags ["item" "form"])))
...
(deftest and-or-nilable-map-of-tuple
  (let [and-node (spec-alpha/contract ::guarded)
    ...
    (is (= "millstrand.api.spec.alpha-test/plain-map?" (get-in and-node ["shape" "form"])))
    ...
    (is (= "millstrand.api.spec.alpha-test/plain-str?" (get-in nil-node ["of" "form"])))
    (is (= "map-of" (get map-node "kind")))
    (is (= "millstrand.api.spec.alpha-test/plain-str?" (get-in map-node ["key" "form"])))
    (is (= "millstrand.api.spec.alpha-test/plain-int?" (get-in map-node ["value" "form"])))
    (is (= ["millstrand.api.spec.alpha-test/plain-str?" "millstrand.api.spec.alpha-test/plain-int?"]
...
(deftest recursive-reference-emits-ref-node
  (let [node (spec-alpha/contract ::tree)
    ...
    (is (= {"kind" "ref" "spec" "millstrand.api.spec.alpha-test/tree"}
...
(deftest spec-forms-keeps-v1-shape-and-accretes-doc
  (let [graph (spec-alpha/spec-forms ::outer)
    ...
    (is (= "millstrand.api.spec.alpha-test/outer" (get root "spec")))
    ...
    (is (= "First line of documentation."
           (get (get by-spec "millstrand.api.spec.alpha-test/qualified-key") "doc")))
    (is (true? (get (get by-spec "millstrand.api.spec.alpha-test/hidden") "private")))
    (is (contains? by-spec "millstrand.api.spec.alpha-test/tracked"))))
...
(deftest template-renders-copyable-skeleton
  (let [skeleton (spec-alpha/template ::outer)]
    (is (map? skeleton))
    (is (contains? skeleton "millstrand.api.spec.alpha-test/qualified-key"))
    (is (map? (get skeleton "nested")))
    (is (= ["<millstrand.api.spec.alpha-test/plain-str?>"] (get skeleton "tags")))
...
(deftest template-structural-nodes
  (is (= "<millstrand.api.spec.alpha-test/plain-str?>" (spec-alpha/template ::either)))
  (is (= "<millstrand.api.spec.alpha-test/plain-str?>" (spec-alpha/template ::maybe)))
  (is (= ["<millstrand.api.spec.alpha-test/plain-str?>" "<millstrand.api.spec.alpha-test/plain-int?>"]
         (spec-alpha/template ::pair)))
  (is (= {"<millstrand.api.spec.alpha-test/plain-str?>" "<millstrand.api.spec.alpha-test/plain-int?>"}
         (spec-alpha/template ::lookup)))
  (is (= "<millstrand.api.spec.alpha-test/plain-map?>" (spec-alpha/template ::guarded)))
  (let [tree (spec-alpha/template ::tree)]
    (is (= "<recursive: millstrand.api.spec.alpha-test/tree>"
...
(deftest problems-detect-missing-keys-structurally
  (let [missing (spec-alpha/problems ::nested {})
    ...
    (is (= "millstrand.api.spec.alpha-test/qualified-key"
...
(deftest projection-bundle-and-json-safety
  (let [bundle (spec-alpha/projection ::outer)]
    ...
    (is (= "millstrand.api.spec.alpha-test/outer" (get bundle "spec")))
...
(deftest unregistered-spec-fails-loudly
  (let [failure (is (thrown? clojure.lang.ExceptionInfo
    ...
    (is (contains? (ex-data failure) :millstrand.api.spec.alpha/error)))
