(ns millstrand.core.weaver.patterns-test
...
(defn drain-events!
  ...
  (let [signal (promise)]
    ...
    (events/register-handler! rt :event-drain #{:test/event-drain}
                              'millstrand.core.weaver.patterns-test/event-drain-handler {})
    (try
...
(defn snapshot-probe-op-v1
  ...
  (weaver/replace-op! runtime 'snapshot-probe raw-mutating-standard
                      'millstrand.core.weaver.patterns-test/snapshot-probe-op-v2)
  {:version :v1})
...
(s/def :millstrand.core.weaver.patterns-test.plan/feature plan-non-blank?)
(s/def :millstrand.core.weaver.patterns-test.plan/title plan-non-blank?)
(s/def :millstrand.core.weaver.patterns-test.plan/key plan-non-blank?)
(s/def :millstrand.core.weaver.patterns-test.plan/body plan-non-blank?)
(s/def :millstrand.core.weaver.patterns-test.plan/kind #{"task" "review"})
(s/def :millstrand.core.weaver.patterns-test.plan/hitl boolean?)
(s/def :millstrand.core.weaver.patterns-test.plan/depends_on
  (s/coll-of :millstrand.core.weaver.patterns-test.plan/key :kind vector?))
(s/def :millstrand.core.weaver.patterns-test.plan/max-attempts pos-int?)
(s/def :millstrand.core.weaver.patterns-test.plan/task
  (s/keys :req-un [:millstrand.core.weaver.patterns-test.plan/key :millstrand.core.weaver.patterns-test.plan/title]
          :opt-un [:millstrand.core.weaver.patterns-test.plan/body :millstrand.core.weaver.patterns-test.plan/kind
                   :millstrand.core.weaver.patterns-test.plan/hitl :millstrand.core.weaver.patterns-test.plan/depends_on
                   :millstrand.core.weaver.patterns-test.plan/max-attempts]))
(s/def :millstrand.core.weaver.patterns-test.plan/tasks
  (s/coll-of :millstrand.core.weaver.patterns-test.plan/task :kind vector? :min-count 1))
(s/def :millstrand.core.weaver.patterns-test.plan/input
  (s/and map?
         #(every? #{:feature :title :tasks :body} (keys %))
         (s/keys :req-un [:millstrand.core.weaver.patterns-test.plan/feature :millstrand.core.weaver.patterns-test.plan/title
                          :millstrand.core.weaver.patterns-test.plan/tasks]
                 :opt-un [:millstrand.core.weaver.patterns-test.plan/body])))
(s/def :millstrand.core.weaver.patterns-test.pipeline/input
  (s/and map?
         #(s/valid? :millstrand.core.weaver.patterns-test.plan/tasks (:tasks %))))

(deftest weaver-pattern-registry-and-weave
  (with-runtime
    (fn [rt _]
      (weaver/init rt)
      (is (= {:name "dev-task" :fn 'millstrand.core.weaver.patterns-test/test-pattern :input-spec ::pattern-input}
             (patterns/register-pattern! rt 'dev-task 'millstrand.core.weaver.patterns-test/test-pattern ::pattern-input)))
      (is (= [{:name "dev-task" :fn 'millstrand.core.weaver.patterns-test/test-pattern :input-spec ::pattern-input}]
             (patterns/patterns rt)))
      (is (= {:name "documented-task"
              :doc "Create implementation and review strands."
              :fn 'millstrand.core.weaver.patterns-test/test-pattern
              :input-spec ::pattern-input}
             (patterns/register-pattern! rt 'documented-task "Create implementation and review strands."
                                         'millstrand.core.weaver.patterns-test/test-pattern ::pattern-input)))
      (is (= "Create implementation and review strands."
             (:doc (patterns/explain rt :documented-task))))
      (is (thrown-with-msg? clojure.lang.ExceptionInfo
                            #"Pattern doc"
                            (patterns/register-pattern! rt 'bad-doc "" 'millstrand.core.weaver.patterns-test/test-pattern ::pattern-input)))
      (is (thrown-with-msg? clojure.lang.ExceptionInfo
                            #"callable"
                            (patterns/register-pattern! rt 'bad-fn
                                                        'millstrand.core.weaver.patterns-test/not-callable-hook
                                                        ::pattern-input)))
      (is (thrown-with-msg? clojure.lang.ExceptionInfo
                            #"Pattern not registered; cannot replace"
                            (patterns/replace-pattern! rt 'absent 'millstrand.core.weaver.patterns-test/test-pattern
                                                       ::pattern-input)))
      (is (= {:name "documented-task"
              :fn 'millstrand.core.weaver.patterns-test/test-pattern
              :input-spec ::pattern-input}
             (patterns/replace-pattern! rt 'documented-task 'millstrand.core.weaver.patterns-test/test-pattern
                                        ::pattern-input))
      ...
      (events/register-handler! rt :capture-weave #{:batch/applied}
                                'millstrand.core.weaver.patterns-test/capture-event {})
...
(deftest pattern-input-projection-benchmarks
  (with-runtime
    (fn [rt _]
      (weaver/init rt)
      (patterns/register-pattern! rt 'agent-plan-like 'millstrand.core.weaver.patterns-test/test-pattern
                                  :millstrand.core.weaver.patterns-test.plan/input)
      (patterns/register-pattern! rt 'pipeline-like 'millstrand.core.weaver.patterns-test/test-pattern
                                  :millstrand.core.weaver.patterns-test.pipeline/input)
...
(deftest weaver-weave-runs-create-only-batch-hooks-once-and-normalizes-attributes
  (with-runtime
    (fn [rt _]
      ...
      (reset! hook-contexts [])
      (patterns/register-pattern! rt 'dev-task 'millstrand.core.weaver.patterns-test/test-pattern ::pattern-input)
      (patterns/register-pattern! rt 'points 'millstrand.core.weaver.patterns-test/points-pattern ::pattern-input)
      (hooks/register-hook! rt :parse #{:attributes/normalize} 'millstrand.core.weaver.patterns-test/parse-story-points-hook {})
      (hooks/register-hook! rt :capture-batch #{:batch/apply-before-commit} 'millstrand.core.weaver.patterns-test/capture-hook {})
...
(deftest weaver-pattern-failures-validate-before-code-and-rollback
  (with-runtime
    (fn [rt _]
      ...
      (reset! hook-contexts [])
      (reset! delivered-events [])
      (events/register-handler! rt :capture #{:strand/added :batch/applied} 'millstrand.core.weaver.patterns-test/capture-event {})
      (patterns/register-pattern! rt 'counting 'millstrand.core.weaver.patterns-test/counting-pattern ::never-valid)
      (hooks/register-hook! rt :capture-batch #{:batch/apply-before-commit} 'millstrand.core.weaver.patterns-test/capture-hook {})
      (is (thrown-with-msg? clojure.lang.ExceptionInfo
                            #"Pattern input failed spec validation"
      ...
      (patterns/register-pattern! rt 'bad-edge 'millstrand.core.weaver.patterns-test/bad-edge-pattern ::pattern-input)
      (is (thrown-with-msg? clojure.lang.ExceptionInfo
      ...
      (hooks/register-hook! rt :reject-batch #{:batch/apply-before-commit} 'millstrand.core.weaver.patterns-test/rejecting-hook {})
      (try
