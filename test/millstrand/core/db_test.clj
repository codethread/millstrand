(ns millstrand.core.db-test
  "Tests for millstrand.core.db: strand/edge persistence, queries, and SQLite behavior."
...
(defn- private-db-var [symbol]
  (or (ns-resolve 'millstrand.core.db symbol)
...
(deftest bootstrap-ignores-and-preserves-non-millstrand-objects
  (with-raw-db
    (fn [ds]
  ...
  (with-db
    (fn [ds]
      (let [active-blocker (:id (db/add-strand! ds {:title "Active blocker"}))
        ...
        (is (thrown-with-msg? clojure.lang.ExceptionInfo #"valid relation name"
                              (query/compile-query [:edge/out [:param :relation] [:exists :id]] {:relation ["__millstrand_query_param__"]})))))))
...
(deftest init-rejects-old-document-strand-schema
  (let [db-file (temp-db-file)
    ...
    (try
      ...
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"does not match the expected Millstrand schema"
                            (db/init! ds)))
      (is (zero? (pragma-value ds "user_version")))
