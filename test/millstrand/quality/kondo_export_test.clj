...
(def ^:private consumer-source
  (str
   ...
   "  {:arg-spec {:op \"echo\" :doc \"Echo an argument.\"}}\n"
   "  [argv]\n"
   "  argv)\n"
   "(millstrand/defquery active \"Find active strands.\" {}\n"
   "  [:= :state \"active\"])\n"
   "(millstrand/defpattern task \"Create a task.\" {:spec ::task}\n"
   "  [input]\n"
   "  input)\n"
   "(millstrand/defhook validate \"Validate a change.\" {:types #{:strand/added}}\n"
   "  [event]\n"
   "  event)\n"
   "(millstrand/defhandler report \"Report a change.\" {:types #{:strand/added}}\n"
   "  [event]\n"
   "  event)\n"
   "(millstrand/defbin tool \"Run a tool.\" {:executable \"tool\"})\n"
   "\n"
