(ns millstrand.source-root-spools-test
  "Focused tests for the `{:millstrand/source-root \"spools/<name>\"}` spool coordinate
...
(deftest source-root-rejects-non-relative-and-mixed-kind-shapes
  (testing "absolute path fails structural validation"
    (is (thrown-with-msg? clojure.lang.ExceptionInfo #"relative :millstrand/source-root"
                          (spool-sync/validate-shared-spools-config!
                           "/tmp/spools.edn" {:spools {'demo {:millstrand/source-root "/abs/spools/demo"}}}))))
  (testing "leading ~ fails structural validation"
    (is (thrown-with-msg? clojure.lang.ExceptionInfo #"relative :millstrand/source-root"
                          (spool-sync/validate-shared-spools-config!
                           "/tmp/spools.edn" {:spools {'demo {:millstrand/source-root "~/spools/demo"}}}))))
  (testing ".. segment fails structural validation"
    (is (thrown-with-msg? clojure.lang.ExceptionInfo #"relative :millstrand/source-root"
                          (spool-sync/validate-shared-spools-config!
                           "/tmp/spools.edn" {:spools {'demo {:millstrand/source-root "spools/../etc"}}}))))
  (testing "mixing source-root with a local coordinate fails before resolution"
    (is (thrown-with-msg? clojure.lang.ExceptionInfo #"exactly one coordinate kind"
                          (spool-sync/validate-shared-spools-config!
                           "/tmp/spools.edn"
                           {:spools {'demo {:millstrand/source-root "spools/demo" :local/root "spools/demo"}}}))))
  (testing "mixing source-root with a git coordinate fails before resolution"
    (is (thrown-with-msg? clojure.lang.ExceptionInfo #"exactly one coordinate kind"
                          (spool-sync/validate-shared-spools-config!
                           "/tmp/spools.edn"
                           {:spools {'demo {:millstrand/source-root "spools/demo"
...
(deftest source-root-normalizes-to-a-single-kind-relevant-coordinate
  (let [normalized (spool-sync/validate-shared-spools-config!
    ...
    (is (= {:kind :millstrand/source-root :millstrand/source-root "spools/demo"}
           (:coordinate family)))
...
(deftest source-root-resolves-beneath-checkout-spools-and-syncs-a-loaded-root
  (with-runtime
    (fn [rt config-dir]
      (let [checkout (test-support/temp-dir "millstrand-source-root-checkout")]
        (write-checkout-spool! checkout "spools/demo" 'demo.core)
        (write-spools! config-dir {:spools {'demo {:millstrand/source-root "spools/demo"}}})
        (with-checkout checkout
          (fn []
            ...
            (let [entry (get-in (spool-sync/approved-spool-syncs rt) [:spools 'demo])
              ...
              (is (= :millstrand/source-root (:kind entry)))
              (is (= "spools/demo" (:millstrand/source-root entry)))
              (is (= root (:root entry)))
...
(deftest source-root-rejects-a-symlink-escaping-the-checkout-spools-dir
  (with-runtime
    (fn [rt config-dir]
      (let [checkout (test-support/temp-dir "millstrand-source-root-checkout")
            outside (test-support/temp-dir "millstrand-source-root-outside")]
        ...
        (write-spools! config-dir {:spools {'evil {:millstrand/source-root "spools/evil"}}})
        (with-checkout checkout
          (fn []
...
(deftest source-root-fails-loudly-when-the-checkout-is-unavailable
  (testing "a non-file classpath resource identifies no readable file checkout"
    (is (nil? (weaver-runtime/source-checkout-root
               (java.net.URL. "http://example.invalid/millstrand/core/weaver/runtime.clj"))))
...
(deftest source-root-key-is-rejected-inside-a-spool-root-deps
  (testing "a :millstrand/source-root source coordinate is rejected loudly in deps.edn :deps"
    (is (thrown-with-msg? clojure.lang.ExceptionInfo
                          #"source-bearing coordinates"
                          (#'spool-sync/validate-maven-coordinate!
                           'some/lib {:millstrand/source-root "spools/x"} {:root "/tmp/root"}))))
      ...
      (fn [rt config-dir]
        (let [checkout (test-support/temp-dir "millstrand-source-root-checkout")
          ...
          (spit (io/file root "deps.edn")
                (pr-str {:paths ["src"] :deps {'evil/lib {:millstrand/source-root "spools/other"}}}))
          (write-spools! config-dir {:spools {'demo {:millstrand/source-root "spools/demo"}}})
          (with-checkout checkout
            (fn []
              (sync! rt)
...
(deftest synced-source-root-provider-wins-over-the-test-classpath-overlap
  (with-runtime
    (fn [rt config-dir]
      (let [checkout (test-support/temp-dir "millstrand-source-root-checkout")
            batteries-file (io/file checkout "spools" "batteries" "src" "millstrand" "spools" "batteries.clj")]
        ...
        (spit batteries-file "(ns millstrand.spools.batteries)\n")
        (spit (io/file checkout "spools" "batteries" "deps.edn") "{:paths [\"src\"]}\n")
        (write-spools! config-dir {:spools {'millstrand.spools/batteries {:millstrand/source-root "spools/batteries"}}})
        (with-checkout checkout
          (fn []
            (sync! rt)
            (let [classpath-resource (io/resource "millstrand/spools/batteries.clj")
                  synced-path (.getCanonicalPath batteries-file)
                  resolved (@#'module-refresh/ns-source-file rt 'millstrand.spools.batteries)]
