(ns millstrand.quality.conventions-check-test
  ...
  spool sources use `millstrand.core.*` only from unsafe-named namespaces
...
(deftest api-test-boundary-permits-public-fixtures-and-any-suite-size
  (with-modules
    {"ok" (str "(ns millstrand.api.example.alpha-test\n"
               "  (:require [millstrand.core.specs :as specs]\n"
...
(deftest api-test-boundary-rejects-core-implementation-use
  (with-modules
    {"bad" (str "(ns millstrand.api.example.alpha-test\n"
                "  (:require [millstrand.core.db :as db]))\n"
                "(defn probe [] (db/pending-wakes nil))\n")}
    (fn [dirs]
      (let [findings (api-tests/findings (.getPath (dirs "bad")))]
        (is (= 2 (count findings)))
        (is (some #(re-find #"direct millstrand.core implementation use.*millstrand.core.db" %) findings))
        (is (some #(re-find #"direct millstrand.core implementation use.*db/pending-wakes" %) findings))))))

(deftest api-test-boundary-reports-private-vars-redefs-and-megasuites
  (with-modules
    {"bad" (str "(ns millstrand.api.example.alpha-test\n"
                "  (:require [millstrand.test.alpha :as t]\n"
                "            [millstrand.spools-test :as mega]))\n"
                "(ns-resolve 'millstrand.core.weaver.runtime 'secret)\n"
                "(with-redefs [millstrand.core.db/query! identity] (t/with-weaver-world []))\n")}
    (fn [dirs]
      (let [findings (api-tests/findings (.getPath (dirs "bad")))]
        (is (= 3 (count findings)))
        (is (some #(re-find #"private core Var resolution.*millstrand.core.weaver.runtime" %) findings))
        (is (some #(re-find #"core collaborator redefinition.*millstrand.core.db/query!" %) findings))
        (is (some #(re-find #"test-megasuite require.*millstrand.spools-test" %) findings))))))
...
(deftest workspace-test-namespace-and-directory-map-bidirectionally
  (let [valid {:name 'millstrand.ct.config-test
    ...
    (testing "the workspace directory cannot carry an unrelated namespace"
      (let [[finding] (check-workspace-analysis
                       {:namespace-definitions
                        [(assoc valid :name 'millstrand.config-test)]})]
        (is (str/includes? finding "must declare a `millstrand.ct.*` namespace"))))
      ...
      (fn [dirs]
        (let [dir (dirs "generic")
          ...
          (is (empty? (workspace-tests/check
                       {:namespace-definitions
                        [{:name 'millstrand.generic-test :filename file :row 1}]}
                       (.getPath dir)))))))))
...
(deftest internal-namespaces-never-require-alpha
  (let [usage {:from 'millstrand.api.tidy.internal :to 'millstrand.api.other.alpha
               :filename "src/millstrand/api/tidy/internal.clj" :row 3}
...
(deftest only-own-alpha-reaches-internal
  (testing "a foreign src namespace requiring internal is a finding"
    (let [usage {:from 'millstrand.core.weaver :to 'millstrand.api.tidy.internal
                 :filename "src/millstrand/core/weaver.clj" :row 3}
          findings (api-form/findings {:namespace-usages [usage]} {} #{})]
      (is (= 1 (count findings)))
      (is (re-find #"only the module's own alpha" (first findings)))))
  (testing "absolute kondo filenames still hit the source-tier rule"
    (let [usage {:from 'millstrand.core.weaver :to 'millstrand.api.tidy.internal
                 :filename "/abs/repo/src/millstrand/core/weaver.clj" :row 3}]
  ...
  (testing "the module's own alpha, internal siblings, and tests are allowed"
    (let [own {:from 'millstrand.api.tidy.alpha :to 'millstrand.api.tidy.internal
               :filename "src/millstrand/api/tidy/alpha.clj" :row 3}
          nested {:from 'millstrand.api.tidy.alpha :to 'millstrand.api.tidy.internal.validate
                  :filename "src/millstrand/api/tidy/alpha.clj" :row 4}
          sibling {:from 'millstrand.api.tidy.internal.validate
  ...
  (testing "a foreign module's internal is still fenced, nested or not"
    (let [usage {:from 'millstrand.api.other.internal.helpers
                 :to 'millstrand.api.tidy.internal.validate
                 :filename "src/millstrand/api/other/internal/helpers.clj" :row 3}]
      (is (= 1 (count (api-form/findings {:namespace-usages [usage]} {} #{})))))))
...
(deftest core-usage-from-safe-namespace-is-a-finding
  (testing "a require and a qualified var usage are each findings"
    (let [findings (spool-tiers/findings
                    {:namespace-usages [(spool-usage "tidy" 'millstrand.spools.tidy
                                                     'millstrand.core.db 5)]
                     :var-usages [(spool-usage "tidy" 'millstrand.spools.tidy
                                               'millstrand.core.weaver.module-refresh
                                               40)]}
                    #{"tidy"})]
      (is (= 2 (count findings)))
      (is (every? #(re-find #"spools build on `millstrand.api" %) findings))))
  (testing "absolute kondo filenames still hit the rule"
    (let [usage (assoc (spool-usage "tidy" 'millstrand.spools.tidy 'millstrand.core.db 5)
                       :filename "/abs/repo/spools/tidy/src/millstrand/spools/tidy.clj")]
      (is (= 1 (count (spool-tiers/findings {:namespace-usages [usage]}
                                            #{"tidy"})))))))

(deftest core-usage-from-unsafe-named-namespace-is-permitted
  (let [analysis {:namespace-definitions
                  [(spool-ns-def "unsafe-tidy" 'millstrand.spools.unsafe-tidy
                                 "UNSAFE: reads millstrand.core.db directly.")]
                  :namespace-usages
                  [(spool-usage "unsafe-tidy" 'millstrand.spools.unsafe-tidy
                                'millstrand.core.db 5)]}]
...
(deftest unsafe-marker-matches-segments-not-substrings
  (testing "a substring hit grants nothing"
    (let [analysis {:namespace-usages
                    [(spool-usage "tidy" 'millstrand.spools.notunsafe
                                  'millstrand.core.db 5)]}]
      (is (= 1 (count (spool-tiers/findings analysis #{"tidy"}))))))
  (testing "a bare `unsafe` segment is a marker"
    (is (spool-tiers/unsafe-ns? 'millstrand.spools.tidy.unsafe))
    (is (not (spool-tiers/unsafe-ns? 'millstrand.spools.unsafely)))))

(deftest core-usage-outside-shipped-spool-sources-is-out-of-scope
  (let [engine {:from 'millstrand.core.weaver :to 'millstrand.core.db
                :filename "src/millstrand/core/weaver.clj" :row 3}
        test-use {:from 'millstrand.spools.tidy-test :to 'millstrand.core.db
                  :filename "spools/tidy/test/millstrand/spools/tidy_test.clj" :row 3}]
...
(deftest own-spool-unsafe-require-is-permitted-cross-spool-is-not
  (let [defs [(spool-ns-def "tidy" 'millstrand.spools.tidy.unsafe-db
                            "UNSAFE: reads millstrand.core.db directly.")
              (spool-ns-def "tidy" 'millstrand.spools.tidy "Doc.")
              (spool-ns-def "other" 'millstrand.spools.other "Doc.")]
        core-use (spool-usage "tidy" 'millstrand.spools.tidy.unsafe-db
                              'millstrand.core.db 5)
        own-use (spool-usage "tidy" 'millstrand.spools.tidy
                             'millstrand.spools.tidy.unsafe-db 3)
        foreign-use (spool-usage "other" 'millstrand.spools.other
                                 'millstrand.spools.tidy.unsafe-db 3)]
...
(deftest stale-unsafe-name-is-a-finding
  (let [analysis {:namespace-definitions
                  [(spool-ns-def "tidy" 'millstrand.spools.tidy.unsafe-db
                                 "UNSAFE: reads millstrand.core.db directly.")]
                  :namespace-usages
                  [(spool-usage "tidy" 'millstrand.spools.tidy.unsafe-db
                                'millstrand.api.graph.alpha 5)]}]
    (is (= 1 (count (spool-tiers/findings analysis #{"tidy"}))))
...
(deftest unsafe-docstring-lead-agrees-with-the-name
  (testing "unsafe-named ns without the UNSAFE: lead is a finding"
    (let [analysis {:namespace-definitions
                    [(spool-ns-def "tidy" 'millstrand.spools.tidy.unsafe-db "Doc.")]
                    :namespace-usages
                    [(spool-usage "tidy" 'millstrand.spools.tidy.unsafe-db
                                  'millstrand.core.db 5)]}
  ...
  (testing "safe ns claiming UNSAFE: in its docstring is a finding"
    (let [analysis {:namespace-definitions
                    [(spool-ns-def "tidy" 'millstrand.spools.tidy
...
(defn- def-spool-sites
  ...
  (let [file (io/file (.toFile (Files/createTempDirectory
                      ...
                      "spool.clj")]
    (spit file content)
    (map #(assoc % :filename "spools/tidy/src/millstrand/spools/tidy.clj")
...
(deftest unreadable-file-site-is-surfaced-as-a-finding
  (let [findings (spool-var/findings
                  [{:filename "spools/tidy/src/millstrand/spools/tidy.clj"
