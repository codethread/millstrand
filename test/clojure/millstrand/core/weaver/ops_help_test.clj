(ns millstrand.core.weaver.ops-help-test
  ...
  (:require [clojure.java.io :as io]
            ...
            [millstrand.core.weaver.help :as weaver-help]
            [millstrand.core.weaver.module-publication :as module-publication]
    ...
    (with-runtime
      (fn [rt _]
        (let [{:keys [entries missing required unchecked]}
              (owner-return-coverage rt 'millstrand.core.weaver.help #{})]
    ...
    (with-runtime
      (fn [rt _]
        ...
        (let [initial (owner-return-coverage rt 'millstrand.core.weaver.ops-help-test #{})
          ...
          (let [partial (owner-return-coverage rt 'millstrand.core.weaver.ops-help-test @checked)]
          ...
          (let [{:keys [missing unchecked]}
                (owner-return-coverage rt 'millstrand.core.weaver.ops-help-test @checked)]
            (is (empty? missing))
            (is (empty? unchecked))))))))
  ...
  (with-runtime
    (fn [rt _]
        ...
        (doseq [[name opts reason]
          ...
          (let [before (weaver/ops rt)
                e (is (thrown? clojure.lang.ExceptionInfo
                               (weaver/register-op! rt name opts 'millstrand.core.weaver.ops-help-test/test-op)))]
      ...
      (testing "unknown metadata keys fail loudly"
        (is (thrown-with-msg? clojure.lang.ExceptionInfo
                              #"unknown keys"
                              (weaver/register-op! rt 'nope {:bogus true} 'millstrand.core.weaver.ops-help-test/test-op))))
    ...
    (fn [rt _]
      ...
      (testing "re-registering a name fails loudly, naming both provenances"
        (let [e (is (thrown? clojure.lang.ExceptionInfo
          ...
          (is (= 'millstrand.peers-test (:attempted-provenance (ex-data e))))))
    ...
    (fn [rt _]
      ...
      (testing "subcommand map results receive the canonical operation label"
        (let [subcommands (into {}
          ...
          (weaver/register-op! rt :result-labels
                               ...
                               'millstrand.core.weaver.ops-help-test/subcommand-result-op)
                                         ...
                                         {"task" {:doc "Manage tasks"
                                                  ...
                                                  {"absent" {:hook-class :read
                                                   ...
                                                   "equal" {:hook-class :read
                                                            :deadline-class :standard}}}}}}
                             'millstrand.core.weaver.ops-help-test/two-level-command-result-op)
        ...
        (weaver/register-op! rt 'subcommand-failure
                             {:arg-spec {:op "subcommand-failure"
                                         :subcommands {"run" {:doc "Fail"
                                                              :hook-class :mutating
                                                              :deadline-class :standard}}}}
                             'millstrand.core.weaver.ops-help-test/throwing-op)
        (let [e (is (thrown-with-msg? clojure.lang.ExceptionInfo
                                      #"op blew up"
                             ...
                             {:stream? true
                              :arg-spec {:op "streaming-subcommand"
                                         :subcommands {"run" {:doc "Stream"
                                                              :hook-class :mutating
                                                              :deadline-class :unbounded}}}}
                             'millstrand.core.weaver.ops-help-test/streaming-subcommand-op)
        (let [emitted (atom [])
      ...
      (testing "deep grammars route, label, and fail with the canonical context (MI8)"
        (weaver/register-op!
         ...
         'millstrand.core.weaver.ops-help-test/deep-path-result-op)
  ...
  (with-runtime
    (fn [rt _]
      ...
      (core-registry/put-entry!
       ...
       {:name "unclassed"
        :fn 'millstrand.core.weaver.ops-help-test/context-echo-op
        :stream? false
        :provenance 'millstrand.core.weaver.ops-help-test
...
(defn- register-fixture-outcomes!
  ...
  (doseq [name names]
    (glossary/register-glossary-outcome!
     rt {:name name :definition (str name " definition") :owner 'millstrand.core.weaver.ops-help-test/fixture})))
    ...
    (fn [rt _]
      ...
      (testing "help embeds the projection fields on spec-declaring args (SPEC-003.C23c)"
        (let [node (:node (weaver/op! rt 'help ["chooser"]))
          ...
          (is (= "millstrand.core.weaver.ops-help-test/choice-input" (:spec input)))
          ...
          (is (= "millstrand.core.weaver.ops-help-test/choice-input"
                 (get-in input [:spec-forms 0 "spec"])))
          (testing "a positional's declared enum spec projects too"
            (is (= "millstrand.core.weaver.ops-help-test/choice-name" (:spec choice)))
