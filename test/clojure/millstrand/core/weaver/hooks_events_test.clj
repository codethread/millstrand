(ns millstrand.core.weaver.hooks-events-test
...
(defn drain-events!
  ...
  (let [signal (promise)]
    ...
    (events/register-handler! rt :event-drain #{:test/event-drain}
                              'millstrand.core.weaver.hooks-events-test/event-drain-handler {})
    (try
...
(s/def :millstrand.core.weaver.hooks-events-test.plan/feature plan-non-blank?)
(s/def :millstrand.core.weaver.hooks-events-test.plan/title plan-non-blank?)
(s/def :millstrand.core.weaver.hooks-events-test.plan/key plan-non-blank?)
(s/def :millstrand.core.weaver.hooks-events-test.plan/body plan-non-blank?)
(s/def :millstrand.core.weaver.hooks-events-test.plan/kind #{"task" "review"})
(s/def :millstrand.core.weaver.hooks-events-test.plan/hitl boolean?)
(s/def :millstrand.core.weaver.hooks-events-test.plan/depends_on
  (s/coll-of :millstrand.core.weaver.hooks-events-test.plan/key :kind vector?))
(s/def :millstrand.core.weaver.hooks-events-test.plan/max-attempts pos-int?)
(s/def :millstrand.core.weaver.hooks-events-test.plan/task
  (s/keys :req-un [:millstrand.core.weaver.hooks-events-test.plan/key :millstrand.core.weaver.hooks-events-test.plan/title]
          :opt-un [:millstrand.core.weaver.hooks-events-test.plan/body :millstrand.core.weaver.hooks-events-test.plan/kind
                   :millstrand.core.weaver.hooks-events-test.plan/hitl :millstrand.core.weaver.hooks-events-test.plan/depends_on
                   :millstrand.core.weaver.hooks-events-test.plan/max-attempts]))
(s/def :millstrand.core.weaver.hooks-events-test.plan/tasks
  (s/coll-of :millstrand.core.weaver.hooks-events-test.plan/task :kind vector? :min-count 1))
(s/def :millstrand.core.weaver.hooks-events-test.plan/input
  (s/and map?
         #(every? #{:feature :title :tasks :body} (keys %))
         (s/keys :req-un [:millstrand.core.weaver.hooks-events-test.plan/feature :millstrand.core.weaver.hooks-events-test.plan/title
                          :millstrand.core.weaver.hooks-events-test.plan/tasks]
                 :opt-un [:millstrand.core.weaver.hooks-events-test.plan/body])))
(s/def :millstrand.core.weaver.hooks-events-test.pipeline/input
  (s/and map?
         #(s/valid? :millstrand.core.weaver.hooks-events-test.plan/tasks (:tasks %))))

(deftest weaver-event-runtime-registers-dispatches-and-records-failures
  (with-runtime
    (fn [rt _]
      (reset! delivered-events [])
      (let [entry (events/register-handler! rt :capture #{:strand/added} 'millstrand.core.weaver.hooks-events-test/capture-event {:purpose :test})]
        (is (= {:key :capture
                :types #{:strand/added}
                :fn 'millstrand.core.weaver.hooks-events-test/capture-event
        ...
        (is (= {:key :capture
               ...
               (events/register-handler! rt :capture #{:strand/updated} 'millstrand.core.weaver.hooks-events-test/capture-event {:purpose :replacement})))
        (is (= [] @delivered-events))
        ...
        (events/register-handler! rt :fails #{:strand/updated} 'millstrand.core.weaver.hooks-events-test/failing-event {})
        ...
        (let [failure (last (events/recent-failures rt))]
          ...
          (is (= 'millstrand.core.weaver.hooks-events-test/failing-event (:handler/fn failure)))
...
(deftest weaver-supersession-emits-semantic-event
  (with-runtime
    (fn [rt _]
      (weaver/init rt)
      (reset! delivered-events [])
      (events/register-handler! rt :capture #{:strand/superseded} 'millstrand.core.weaver.hooks-events-test/capture-event {})
...
(deftest strand-supersede-pre-commit-hook-inspects-and-rejects-atomically
  (with-runtime
    (fn [rt _]
      ...
      (reset! delivered-events [])
      (events/register-handler! rt :capture #{:strand/superseded} 'millstrand.core.weaver.hooks-events-test/capture-event {})
      (hooks/register-hook! rt :capture-supersede #{:strand/supersede-before-commit} 'millstrand.core.weaver.hooks-events-test/capture-hook {})
      (let [old (weaver/add! rt {:title "Old"})
            replacement (weaver/add! rt {:title "Replacement"})
            dependent (weaver/add! rt {:title "Dependent"})]
        ...
        (let [reject-old (weaver/add! rt {:title "Reject old"})
          ...
          (reset! delivered-events [])
          (hooks/register-hook! rt :reject-supersede #{:strand/supersede-before-commit} 'millstrand.core.weaver.hooks-events-test/rejecting-hook {})
          (try
...
(deftest weaver-strand-mutations-emit-events-after-success
  (with-runtime
    (fn [rt _]
      (weaver/init rt)
      (reset! delivered-events [])
      (events/register-handler! rt :capture #{:strand/added :strand/updated :strand/burned} 'millstrand.core.weaver.hooks-events-test/capture-event {})
      (let [added (weaver/add! rt {:title "Evented" :attributes {:owner "agent"}})
        ...
        (is (= :millstrand.api.weaver.alpha (:event/source add-event)))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (events/register-handler! rt :cleanup-temporary #{:strand/updated}
                                'millstrand.core.weaver.hooks-events-test/burn-temporary-children-on-inactive-parent
...
(deftest event-handler-slowness-and-failure-do-not-fail-original-mutation
  (with-runtime
    (fn [rt _]
      ...
      (reset! handler-started (promise))
      (reset! handler-release (promise))
      (events/register-handler! rt :slow #{:strand/updated} 'millstrand.core.weaver.hooks-events-test/slow-capture-event {})
      (events/register-handler! rt :fails #{:strand/updated} 'millstrand.core.weaver.hooks-events-test/failing-event {})
    ...
    (fn [rt _]
      ...
      (let [existing-b (weaver/add! rt {:title "Existing B" :attributes {:owner "agent"}})
        ...
        (events/register-handler! rt :capture #{:batch/applied :strand/added :strand/updated :strand/burned}
                                  'millstrand.core.weaver.hooks-events-test/capture-event {})
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [from (weaver/add! rt {:title "From"})
        ...
        (events/register-handler! rt :capture #{:batch/applied :strand/added :strand/updated :strand/burned}
                                  'millstrand.core.weaver.hooks-events-test/capture-event {})
        (hooks/register-hook! rt :capture-batch #{:batch/apply-before-commit} 'millstrand.core.weaver.hooks-events-test/capture-hook {})
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [a (weaver/add! rt {:title "A"})
        ...
        (reset! delivered-events [])
        (reset! hook-contexts [])
        (events/register-handler! rt :capture #{:batch/applied} 'millstrand.core.weaver.hooks-events-test/capture-event {})
        (hooks/register-hook! rt :reject-batch #{:batch/apply-before-commit} 'millstrand.core.weaver.hooks-events-test/rejecting-hook {})
        (let [before (db-test/graph-snapshot (:datasource rt))]
          (try
    ...
    (fn [rt _]
      ...
      (core-registry/put-entry! (:hook-store rt) :declared-module :declared-capture
                                {:key :declared-capture
                                 ...
                                 :fn 'millstrand.core.weaver.hooks-events-test/capture-hook
                                 :order 0
                                 :metadata {}})
      ...
      (core-registry/put-entry! (:hook-store rt) :declared-module :declared-broken
                                {:key :declared-broken
                                 :types #{:strand/add-before-commit}
                                 :fn 'millstrand.core.weaver.hooks-events-test-missing/absent-hook
                                 :order 0
                                 :metadata {}})
      (try
...
(deftest weaver-apply-batch-hooks-normalize-context-and-reject-atomically
  (with-runtime
    (fn [rt _]
      ...
      (events/register-handler! rt :capture #{:batch/applied :strand/added :strand/updated :strand/burned}
                                'millstrand.core.weaver.hooks-events-test/capture-event {})
      (hooks/register-hook! rt :parse #{:attributes/normalize} 'millstrand.core.weaver.hooks-events-test/parse-story-points-hook {})
      (hooks/register-hook! rt :capture-batch #{:batch/apply-before-commit} 'millstrand.core.weaver.hooks-events-test/capture-hook {})
...
(deftest weaver-burn-by-ids-event-captures-pre-delete-rows-and-requested-ids
  (with-runtime
    (fn [rt _]
      (weaver/init rt)
      (reset! delivered-events [])
      (events/register-handler! rt :capture #{:strand/burned} 'millstrand.core.weaver.hooks-events-test/capture-event {})
      (let [a (weaver/add! rt {:title "A"})
            b (weaver/add! rt {:title "B"})
...
(deftest weaver-event-runtime-fails-loudly-on-invalid-registration
  (with-runtime
    (fn [rt _]
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"key" (events/register-handler! rt [] #{:x} 'millstrand.core.weaver.hooks-events-test/capture-event {})))
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"non-empty" (events/register-handler! rt :bad #{} 'millstrand.core.weaver.hooks-events-test/capture-event {})))
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"set" (events/register-handler! rt :bad [:x] 'millstrand.core.weaver.hooks-events-test/capture-event {})))
      ...
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"callable" (events/register-handler! rt :bad #{:x} 'millstrand.core.weaver.hooks-events-test/not-callable-event-handler {})))
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"metadata" (events/register-handler! rt :bad #{:x} 'millstrand.core.weaver.hooks-events-test/capture-event :opaque)))
...
(deftest weaver-hook-registry-registers-replaces-orders-and-unregisters
  (with-runtime
    (fn [rt _]
      (let [entry (hooks/register-hook! rt :capture #{:payload/received} 'millstrand.core.weaver.hooks-events-test/capture-hook {:doc "Capture"})]
        (is (= {:key :capture
        ...
        (let [replacement (hooks/register-hook! rt :capture #{:strand/add-before-commit} 'millstrand.core.weaver.hooks-events-test/capture-hook {:order 10 :doc "Replaced"})
              early (hooks/register-hook! rt "early" #{:payload/received} 'millstrand.core.weaver.hooks-events-test/capture-hook {:order -1})
              peer-a (hooks/register-hook! rt :a #{:payload/received} 'millstrand.core.weaver.hooks-events-test/capture-hook {})
              peer-b (hooks/register-hook! rt :b #{:payload/received} 'millstrand.core.weaver.hooks-events-test/capture-hook {})]
...
(deftest weaver-hook-registry-validates-inputs
  (with-runtime
    (fn [rt _]
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"key" (hooks/register-hook! rt [] #{:x} 'millstrand.core.weaver.hooks-events-test/capture-hook {})))
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"non-empty" (hooks/register-hook! rt :bad #{} 'millstrand.core.weaver.hooks-events-test/capture-hook {})))
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"set" (hooks/register-hook! rt :bad [:x] 'millstrand.core.weaver.hooks-events-test/capture-hook {})))
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"keywords" (hooks/register-hook! rt :bad #{"x"} 'millstrand.core.weaver.hooks-events-test/capture-hook {})))
...
(deftest attribute-normalize-hooks-thread-transform-results-for-add-and-update
  (with-runtime
    (fn [rt _]
      ...
      (reset! hook-contexts [])
      (reset! delivered-events [])
      (events/register-handler! rt :capture #{:strand/added :strand/updated} 'millstrand.core.weaver.hooks-events-test/capture-event {})
      (hooks/register-hook! rt :parse #{:attributes/normalize} 'millstrand.core.weaver.hooks-events-test/parse-story-points-hook {:order 0})
      (hooks/register-hook! rt :flag #{:attributes/normalize} 'millstrand.core.weaver.hooks-events-test/add-normalized-flag-hook {:order 1})
...
(deftest attribute-normalize-hooks-require-wrapper-and-json-compatible-values
  (with-runtime
    (fn [rt _]
      (weaver/init rt)
      (hooks/register-hook! rt :noop #{:attributes/normalize} 'millstrand.core.weaver.hooks-events-test/noop-normalize-hook {})
      (is (= {:a "b"} (:attributes (weaver/add! rt {:title "Noop" :attributes {:a "b"}}))))
      (doseq [[k f] [[:nil 'millstrand.core.weaver.hooks-events-test/nil-normalize-hook]
                     [:plain 'millstrand.core.weaver.hooks-events-test/non-wrapper-normalize-hook]
                     [:invalid 'millstrand.core.weaver.hooks-events-test/invalid-attributes-hook]]]
...
(deftest attribute-normalize-hook-failures-rollback-and-preserve-cause-data
  (with-runtime
    (fn [rt _]
      ...
      (reset! delivered-events [])
      (events/register-handler! rt :capture #{:strand/added :strand/updated} 'millstrand.core.weaver.hooks-events-test/capture-event {})
      (hooks/register-hook! rt :reject #{:attributes/normalize} 'millstrand.core.weaver.hooks-events-test/rejecting-normalize-hook {})
      (try
        ...
        (catch clojure.lang.ExceptionInfo e
          ...
          (is (= 'millstrand.core.weaver.hooks-events-test/rejecting-normalize-hook (:hook/fn (ex-data e))))
          (is (= "policy/rejected" (:hook/cause-code (ex-data e))))
      ...
      (hooks/register-hook! rt :wrapped #{:attributes/normalize} 'millstrand.core.weaver.hooks-events-test/wrapping-rejecting-normalize-hook {})
      (try
...
(deftest strand-pre-commit-hooks-gate-add-update-and-burn
  (with-runtime
    (fn [rt _]
      ...
      (reset! delivered-events [])
      (events/register-handler! rt :capture #{:strand/added :strand/updated :strand/burned} 'millstrand.core.weaver.hooks-events-test/capture-event {})
      (hooks/register-hook! rt :capture-add #{:strand/add-before-commit} 'millstrand.core.weaver.hooks-events-test/capture-hook {})
      (let [created (weaver/add! rt {:title "Hooked" :attributes {:owner "agent"}})
        ...
        (is (= 'millstrand.core.weaver.hooks-events-test/capture-hook (:hook/fn add-context)))
        ...
        (hooks/register-hook! rt :reject-add #{:strand/add-before-commit} 'millstrand.core.weaver.hooks-events-test/rejecting-hook {})
        (try
          ...
          (catch clojure.lang.ExceptionInfo e
            ...
            (is (= 'millstrand.core.weaver.hooks-events-test/rejecting-hook (:hook/fn (ex-data e))))
            (is (= "policy/rejected" (:hook/cause-code (ex-data e))))))
        (t/await-quiescent! rt)
        ...
        (let [target (weaver/add! rt {:title "Target"})]
          ...
          (reset! hook-contexts [])
          (reset! delivered-events [])
          (hooks/register-hook! rt :capture-update #{:strand/update-before-commit} 'millstrand.core.weaver.hooks-events-test/capture-hook {})
          (let [patch {:title "Updated"
            ...
            (is (= 'millstrand.core.weaver.hooks-events-test/capture-hook (:hook/fn update-context)))
            ...
            (is (= :strand/updated (:event/type update-event)))
            (is (= updated (:strand/after update-event)))
            (hooks/register-hook! rt :reject-update #{:strand/update-before-commit} 'millstrand.core.weaver.hooks-events-test/rejecting-hook {})
            (try
        ...
        (reset! hook-contexts [])
        (reset! delivered-events [])
        (hooks/register-hook! rt :capture-burn #{:strand/burn-before-commit} 'millstrand.core.weaver.hooks-events-test/capture-hook {})
        (let [requested [(:id created) (:id created)]
          ...
          (is (= 'millstrand.core.weaver.hooks-events-test/capture-hook (:hook/fn burn-context)))
        ...
        (let [burn-target (weaver/add! rt {:title "Burn reject"})
          ...
          (let [burn-target (weaver/show rt (:id burn-target))]
            (t/await-quiescent! rt)
            (reset! delivered-events [])
            (hooks/register-hook! rt :reject-burn #{:strand/burn-before-commit} 'millstrand.core.weaver.hooks-events-test/rejecting-hook {})
            (try
