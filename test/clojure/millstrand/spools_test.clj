(ns millstrand.spools-test
  ...
  (:require [clojure.java.io :as io]
            ...
            [millstrand.spools.test-support :refer [delete-tree! run-git! temp-config-dir with-runtime]]
...
(defn- init-git-repo! [root ns-sym & {:keys [tag annotated-tag]}]
  (.mkdirs root)
  (run-git! root "init")
  (run-git! root "config" "user.email" "millstrand-test@example.invalid")
  (run-git! root "config" "user.name" "Millstrand Test")
        ...
        (with-cache-base
          ...
          (fn []
            ...
            (is (= {:spools {'demo/root {:kind :git
                                         ...
                                         :git/sha sha
                                         :git/tag "v1"
                                         :root (.getPath (io/file cache-dir "millstrand" "spools" sha "nested/spool"))
                                         :source (shared-source config-dir)
                                         :provenance :spools-edn}}}
                   (select-keys (runtime/approved rt) [:spools])))))))))
      ...
      (let [sha "0123456789abcdef0123456789abcdef01234567"]
        (doseq [[field entry]
                [[:git/tag {:git/url "u" :git/sha sha :git/tag "v0"}]
                 ...
                 [:millstrand/min {:git/url "u" :git/sha sha :millstrand/min "v0"}]]]
          (testing (name field)
    ...
    (is (= {:family 'demo/family
           ...
           (#'spool-sync/normalize-shared-family
            ...
            {:git/url "u"
             ...
             :requires {'other/root "v2"}
             :millstrand/min "v1"})))
    (is (s/valid? :millstrand.core.weaver.spool-sync/normalized-family
                  (#'spool-sync/normalize-shared-family
...
(deftest sync-refuses-all-unsatisfied-family-and-millstrand-floors
  (with-runtime
    (fn [rt config-dir]
      (let [sha "0123456789abcdef0123456789abcdef01234567"]
        (write-spools!
...
(deftest runtime-sync-validates-millstrand-minimum-against-its-release-marker
  (let [family 'demo/millstrand-floor
        sha "0123456789abcdef0123456789abcdef01234567"
        write-floor! (fn [config-dir floor]
                       (let [root (write-local-lib! config-dir "millstrand-floor" 'demo.millstrand_floor)]
                         (write-spools! config-dir
                                        (pr-str {:spools {family {:git/url "https://example.invalid/millstrand-floor.git"
                                                                  :git/sha sha
                                                                  :git/tag "v1"
                                                                  :millstrand/min floor}}}))
                         (write-local-spools! config-dir
                                              (pr-str {:spools {family {:local/root "spools/millstrand-floor"
                                                                        :claims "v1"}}}))
                         root))]
      ...
      (with-runtime
        ...
        (fn [rt config-dir]
          ...
          (let [ex (is (thrown? clojure.lang.ExceptionInfo (sync-approved! rt)))]
            ...
            (is (= [{:error :millstrand-below-minimum
                     :spool family
                     :millstrand/min "v2"
                     :running "v1"}]
                   (:findings (ex-data ex))))))))
      ...
      (with-runtime
        (fn [rt config-dir]
          ...
          (let [ex (is (thrown? clojure.lang.ExceptionInfo (sync-approved! rt)))]
            ...
            (is (= [{:family family :millstrand/min "v2"}]
                   (:floors (ex-data ex))))))))
    (testing "unmarked runtime without a declared floor"
      (with-runtime
        (fn [rt config-dir]
          (let [root (write-local-lib! config-dir "no-millstrand-floor" 'demo.no_millstrand_floor)]
            (write-spools! config-dir
                           (pr-str {:spools {family {:local/root "spools/no-millstrand-floor"}}}))
  ...
  (with-runtime
    (fn [rt _]
      (is (= {:key :capture
             ...
             (events/register-handler! rt :capture #{:strand/added} 'millstrand.core.weaver.hooks-events-test/capture-event {:purpose :test})))
      (is (= [:capture] (mapv :key (events/handlers rt))))
      (is (= {:key :capture
              :types #{:strand/updated}
              :fn 'millstrand.core.weaver.hooks-events-test/capture-event
              :metadata {}}
             (events/register-handler! rt :capture #{:strand/updated} 'millstrand.core.weaver.hooks-events-test/capture-event)))
          ...
          (fn []
            ...
            (let [result (get-in (sync-approved! rt) [:spools 'demo/root])]
              (is (= {:lib 'demo/root
                      ...
                      :git/sha sha
                      :git/tag "v1"
                      :root (.getPath (io/file cache-dir "millstrand" "spools" sha "spool"))
                      :source (shared-source config-dir)
                      :status :failed
                      :reason :fetch-failed}
          ...
          (fn []
            ...
            (try
              ...
              (let [result (get-in (sync-approved! rt) [:spools lib])]
                ...
                (is (.isFile (io/file cache-dir "millstrand" "spools" sha "after-initial-cache.txt"))))
              (finally
        ...
        (with-cache-base
          ...
          (fn []
            ...
            (let [result (get-in (sync-approved! rt) [:spools 'demo/unreachable])]
              ...
              (is (= (.getPath (io/file cache-dir "millstrand" "spools" unknown)) (:cache-path result)))
              (is (string? (:stderr result)))
  ...
  (with-runtime
    (fn [rt config-dir]
      (let [suffix (str/replace (str (java.util.UUID/randomUUID)) "-" "")
        ...
        (swap! (:spool-state rt) update ::retained vary-meta assoc :millstrand.runtime/generation "older-generation")
        (let [result (sync-approved! rt)]
...
(deftest delete-tree-removes-symlinks-without-following-them
  (let [base (.toFile (java.nio.file.Files/createTempDirectory "millstrand-delete-tree" (make-array java.nio.file.attribute.FileAttribute 0)))
          ...
          (with-cache-base
            ...
            (fn []
              ...
              (let [results (:spools (sync-approved! rt))]
                ...
                (is (false? (.exists (io/file cache-dir "millstrand" "spools" new-sha))))))))))))
    ...
    (fn [rt config-dir]
      (let [repo (io/file config-dir "mono")
            ...
            sha (do
                  (.mkdirs repo)
                  (run-git! repo "init")
                  (run-git! repo "config" "user.email" "millstrand-test@example.invalid")
                  (run-git! repo "config" "user.name" "Millstrand Test")
        ...
        (with-cache-base
          ...
          (fn []
            ...
            (let [result (get-in (sync-approved! rt) [:spools 'demo/mono])]
              (is (= :loaded (:status result)))
              (is (= :fetched (:fetch result)))
              (is (= (.getPath (io/file cache-dir "millstrand" "spools" sha "nested/spool")) (:root result))))))))))
  ...
  (with-runtime
    (fn [rt config-dir]
      (let [repo (io/file config-dir "family-repo")
            ...
            _ (.mkdirs repo)
            _ (run-git! repo "init")
            _ (run-git! repo "config" "user.email" "millstrand-test@example.invalid")
            _ (run-git! repo "config" "user.name" "Millstrand Test")
        ...
        (with-cache-base
          ...
          (fn []
            ...
            (let [result (get-in (sync-approved! rt) [:spools lib])]
              ...
              (is (= :fetched (:fetch result)))
              (is (= (.getPath (io/file cache-dir "millstrand" "spools" sha "missing/spool"))
                     (:root result))))))))))
    ...
    (fn [rt config-dir]
      (let [suffix (str/replace (str (java.util.UUID/randomUUID)) "-" "")
        ...
        (let [record (last (spool-sync/namespace-load-ledger rt))]
          ...
          (is (s/valid? :millstrand.core.weaver.spool-sync/namespace-load-ledger
