...
(defn- schema-mismatch! [mode found-generation expected-generation missing mismatched]
  (throw (ex-info "Database schema does not match the expected Millstrand schema"
...
(defn- incompatible-generation! [classification found-generation expected-generation]
  (case classification
    :refuse-newer
    (throw (ex-info "This Millstrand binary is too old for the database schema generation"
...
(def ^:private all-archive-keys :millstrand.core.db/all-archive-keys)
...
(defn due-wakes
  ...
  [ds now]
  (require-valid! :millstrand.scheduler-wake/wake-at now "Scheduler wake-at must be a java.time.Instant")
  (execute! ds
...
(defn mark-wake-attempt!
  ...
  (require-valid! :millstrand.scheduler-wake/key key "Scheduler key must be a non-blank string")
