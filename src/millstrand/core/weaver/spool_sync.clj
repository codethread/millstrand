(ns millstrand.core.weaver.spool-sync
  ...
  (:require [clojure.java.io :as io]
            ...
            [millstrand.core.weaver.access :refer [approved-spool-sync-state release-marker
...
(def ^:private git-spool-keys #{:git/url :git/sha :git/tag :roots :requires :millstrand/min})
(def ^:private source-root-spool-keys #{:millstrand/source-root})
...
(s/def :millstrand/min ::release-marker)
(s/def :millstrand/source-root ::root-path)
...
(s/def ::source-root-family-entry
  (s/and #(exact-keys? source-root-spool-keys %)
         (s/keys :req [:millstrand/source-root])))
(s/def ::family-entry (s/or :git ::git-family-entry
                            :local ::local-family-entry
                            :millstrand/source-root ::source-root-family-entry))
(s/def ::overlay-entry
...
(s/def ::coordinate
  (s/or :git (s/and #(= :git (:kind %))
        ...
        :local (s/and #(= :local (:kind %))
                      ...
                      (s/keys :req [:local/root]))
        :millstrand/source-root (s/and #(= :millstrand/source-root (:kind %))
                                       #(exact-keys? #{:kind :millstrand/source-root} %)
                                       (s/keys :req [:millstrand/source-root]))))
(s/def ::roots-map ::roots)
(s/def ::millstrand-min (s/nilable ::release-marker))
...
(s/def ::normalized-family
  (s/keys :req-un [::family ::coordinate ::roots-map ::requires ::millstrand-min
                   ::provenance ::source]
          :opt-un [::claims]))
...
(defn- normalize-shared-family [source family entry]
  ...
  (let [local? (contains? entry :local/root)
        ...
        source-root? (contains? entry :millstrand/source-root)]
    ...
    (cond
      source-root?
      (do
        (when-not (relative-root-path? (:millstrand/source-root entry))
          (throw (ex-info "Source-root spool entry requires a relative :millstrand/source-root path with no ~ or .. segments"
                          (assoc source :family family
                                 :millstrand/source-root (:millstrand/source-root entry) :entry entry))))
        (require-spec! ::family-entry entry "Spool family entry has an invalid shape"
                       (assoc source :family family :entry entry))
        (require-spec!
         ::normalized-family
         {:family family
          :coordinate {:kind :millstrand/source-root :millstrand/source-root (:millstrand/source-root entry)}
          :roots-map {family "."}
          :requires {}
          :millstrand-min nil
          :claims nil
      ...
      (do
        ...
        (when (contains? entry :millstrand/min)
          (validate-marker! (:millstrand/min entry) (assoc source :family family :field :millstrand/min)))
        (let [normalized {:family family
                          ...
                          :millstrand-min (:millstrand/min entry)
                          :claims nil
                          :provenance :spools-edn
                          :source source}]
          (require-spec! ::family-entry entry "Spool family entry has an invalid shape"
   ...
   (fn [families overlay]
     (let [family (:family overlay)
       ...
       (assoc families family
              (merge (select-keys base [:roots-map :requires :millstrand-min]) overlay))))
...
(defn- millstrand-minimum-findings [families running-marker]
  (if running-marker
    (vec
     (for [{:keys [family millstrand-min]} families
           :when (and millstrand-min
                      (< (marker-ordinal running-marker)
                         (marker-ordinal millstrand-min)))]
       {:error :millstrand-below-minimum
        :spool family
        :millstrand/min millstrand-min
        :running running-marker}))
    []))

(defn- pending-millstrand-validations [families running-marker]
  (when-not running-marker
    (vec
     (for [{:keys [family millstrand-min]} families
           :when millstrand-min]
       {:check :millstrand/min
        :spool family
        :millstrand/min millstrand-min
        :status :pending
...
(defn- validate-family-requirements! [families running-marker]
  (when (= :none running-marker)
    (let [floors (vec (keep (fn [{:keys [family millstrand-min]}]
                              (when millstrand-min
                                {:family family :millstrand/min millstrand-min}))
                            families))]
      (when (seq floors)
        (throw (ex-info "Running Millstrand release marker is unavailable for declared floors"
  ...
  (let [findings (into (requirement-findings families)
                       (millstrand-minimum-findings families
    ...
    (pending-millstrand-validations families
                                    (when-not (= :none running-marker)
                                      running-marker))))
...
(def ^:private rejected-spool-deps-keys
  #{:git/url :git/sha :local/root :millstrand/source-root})
...
(defn- kind-shaped-root? [entry]
  (case (:kind entry)
    ...
    :local (and (s/valid? :local/root (:local/root entry))
                (not-any? #(contains? entry %) [:git/url :git/sha :git/tag :millstrand/source-root]))
    :millstrand/source-root (and (s/valid? :millstrand/source-root (:millstrand/source-root entry))
...
(defn- unavailable-marker-findings [floors]
  (mapv (fn [{:keys [family] :as floor}]
          {:error :release-marker-unavailable
           :family family
           :millstrand/min (:millstrand/min floor)})
        floors))
...
(defn declared-spools
  ...
  projection. Omitting `running-marker` leaves Millstrand floor checks pending."
  ([runtime]
...
(defn- source-root-coordinate-root
  "Resolve a `:millstrand/source-root` path beneath `<checkout>/spools`, failing loudly.
...
(defn approved-spools
  ...
  before materialization. When `running-marker` is omitted, declared `:millstrand/min`
                       ...
                       (fn [{:keys [family coordinate roots-map claims provenance source]}]
                         (let [kind (:kind coordinate)
                               coordinate-root (case kind
                                                 :git (io/file (cache-base) "millstrand" "spools" (:git/sha coordinate))
                                                 :local (io/file (canonical-root runtime (:local/root coordinate)))
                                                 :millstrand/source-root (source-root-coordinate-root (:millstrand/source-root coordinate)))]
...
(defn- spool-source-fields [entry]
  (case (:kind entry)
    ...
    :millstrand/source-root (select-keys entry [:millstrand/source-root])))
...
(defn- cache-root-file ^java.io.File [entry]
  (io/file (cache-base) "millstrand" "spools" (:git/sha entry)))
...
(defn- launch-basis
  "Read millstrand's immutable launch basis from the `clojure.basis` property file.
...
(defn- resolve-spool-maven-libs
  "Resolve merged Maven `deps` against millstrand's launch classpath, returning `:added`.

  Reads millstrand's launch libs and `:mvn/repos`/`:mvn/local-repo` from the immutable
  ...
  coordinate carrying resolved `:paths`). Libs millstrand already ships are excluded so
  ...
  re-add coordinates millstrand already ships. This is the sole mockable resolver
  ...
  (if (empty? deps)
    ...
    (let [basis (launch-basis)]
      (when-not basis
        (throw (ex-info (format/reflow
                         "|Cannot resolve spool Maven dependencies: millstrand's launch basis
...
(defn- stale-spool-state-report [runtime]
  (let [current (:generation-id runtime)]
    (vec (for [[key value] @(:spool-state runtime)
               :let [generation (:millstrand.runtime/generation (meta value))]
...
(defn sync-approved-spools
  ...
  Maven deps (with `:mvn-overrides` applied) once against millstrand's launch classpath,
  ...
  A running release marker enables `:millstrand/min` validation. An omitted marker
...
(defn- dependency-ordered-sources
  ...
  requires (`clojure.*`, blessed `millstrand.api.*`, other spools) are edges out of
