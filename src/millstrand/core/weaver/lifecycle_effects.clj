(ns millstrand.core.weaver.lifecycle-effects
  "Internal module lifecycle transition engine."
...
(defn- visit
  ...
  (when-not (contains? @permanent effect-id)
    (when (contains? @temporary effect-id)
...
(defn- base-context
  [runtime module-key effect-id declaration phase refresh-result context]
  (merge context
         {:runtime runtime
...
(defn- apply-effect
  [runtime module-key refresh-result lifecycle-context effect-id declaration callables]
  (let [context #(base-context runtime module-key effect-id declaration %
                               refresh-result lifecycle-context)]
    (try
      (case (:kind declaration)
        :seed
      ...
      (catch Throwable throwable
        {:declaration declaration :callables callables :health :degraded
         :status :degraded :phase (case (:kind declaration)
                                    :resource :open
                                    :apply)
         :error (exception-data throwable)}))))

(defn- remove-effect
  [runtime module-key refresh-result lifecycle-context effect-id retained runtime-stop?]
  (let [{:keys [declaration callables handle]} retained
    ...
    (if (and (= :runtime scope) (not runtime-stop?))
      ...
      (try
        (let [phase (if runtime-stop? :runtime-stop :remove)
              context (base-context runtime module-key effect-id declaration
                                    phase refresh-result lifecycle-context)
              result
              (case (:kind declaration)
                :seed nil
...
(defn- run-removals
  [runtime module-key state refresh-result lifecycle-context removal-order runtime-stop?]
  (reduce
   (fn [{:keys [effects blocked] :as acc} effect-id]
     (let [dependents (set (for [[candidate {:keys [declaration]}] effects
       ...
       (if (seq (set/intersection blocked dependents))
         ...
         (let [result (remove-effect runtime module-key refresh-result
                                     lifecycle-context effect-id
           ...
           (case (:status result)
             ...
             ;; Every successful module-scoped removal drops retained state.
             (-> acc
...
(defn- run-applications
  [runtime module-key resolved-callables declarations refresh-result lifecycle-context
   ...
   (fn [{:keys [halted?] :as acc} effect-id]
     (if halted?
       ...
       (let [declaration (get declarations effect-id)
             result (apply-effect runtime module-key refresh-result lifecycle-context
                                  effect-id declaration
                                  (get resolved-callables effect-id))]
         (-> acc
             (assoc :halted? (= :degraded (:status result)))
             (assoc-in [:effects effect-id] result)
             (assoc-in [:outcomes effect-id]
                       (dissoc result :handle :callables :declaration))))))
   removal-result
...
(defn refresh
  ...
  [{:keys [runtime module-key resolver state declarations changed-kinds context
                               ...
                               (reverse
                                (validate!
                                 (into {}
                                       ...
                                       (:effects state {})))))
        removed (run-removals runtime module-key state refresh-result context
                              removal-order runtime-stop?)
