(ns millstrand.core.weaver.module-graph
...
(def ^:private removed-entry-point-refusal
  ...
  (format/reflow
   "|Module options no longer name entry points. Delete the key from the
    |declaration. Publish registry entries with the millstrand/def* authoring forms
   ...
   (into {effect-id declaration}
         (map (fn [dependency]
                [dependency {:kind :seed
                             :apply 'millstrand.core.weaver.lifecycle-effects/validate!}])
