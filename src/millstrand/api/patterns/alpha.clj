(ns millstrand.api.patterns.alpha
  ...
  (:require [clojure.spec.alpha :as s]
            ...
            [next.jdbc :as jdbc]
            [millstrand.api.batch.alpha :as batch-api]
...
(s/def :millstrand.pattern-explain/name (s/and string? #(not (str/blank? %))))
(s/def :millstrand.pattern-explain/fn (s/and string? #(not (str/blank? %))))
(s/def :millstrand.pattern-explain/input-spec (s/and string? #(not (str/blank? %))))
(s/def :millstrand.pattern-explain/contract map?)
(s/def :millstrand.pattern-explain/template
  (s/or :placeholder string?
        :object (s/map-of string? :millstrand.pattern-explain/template)
        :array (s/coll-of :millstrand.pattern-explain/template :kind vector?)))
(s/def :millstrand.pattern-explain/spec-forms vector?)
(s/def :millstrand.pattern-explain/doc (s/and string? #(not (str/blank? %))))
(s/def ::explain-result
  (s/and (s/keys :req-un [:millstrand.pattern-explain/name
                          :millstrand.pattern-explain/fn
                          :millstrand.pattern-explain/input-spec
                          :millstrand.pattern-explain/contract
                          :millstrand.pattern-explain/template
                          :millstrand.pattern-explain/spec-forms]
                 :opt-un [:millstrand.pattern-explain/doc])
...
(defn- pattern-registration-message
  ...
  (cond
    (not (s/valid? :millstrand.pattern/name name))
    "Pattern name is invalid"
    (not (s/valid? :millstrand.pattern/doc doc))
    "Pattern doc must be a non-blank string"
    (not (s/valid? :millstrand.pattern/fn fn-sym))
    "Pattern function must be a fully qualified symbol"
    (not (s/valid? :millstrand.pattern/input-spec input-spec))
...
(defn- pattern-input-contract
  ...
  `:contract`, `:template`, and `:spec-forms` (`millstrand.api.spec.alpha`)."
  [input-spec]
