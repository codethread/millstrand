(ns millstrand.api.spool.alpha
  ...
  source instead of re-deriving them per file. `millhouse.workflow` is a
...
(defn poll-until!
  "The shared spool-tier long-poll skeleton behind `millhouse.workflow/await!`
  and `millhouse.cron/await-quiescent!`: call `check` (a zero-arg fn) once, test
  ...
  (require-valid! ::clock/clock installed-clock
                  "poll-until! clock must be a millstrand.api.clock.alpha/Clock")
  (when-not (map? opts)
