(ns skein.repl
  "The interactive complement to Skein's module authoring forms.

  Two things live here. The registration verbs are the same
  `register/replace/unregister-*!` matrix the `skein.api.*.alpha` tier
  ...
  `connect!` and the accessors a standalone JVM hands to `skein.core.client`,
  ...
  `skein.api.*.alpha` verbs directly; this namespace is the same capability with
...
;; explicitly through `skein.core.client`, reading the selection back from
...
(defn connected-config-dir
  ...
  `skein.core.client/call-world`. Throws with remediation when nothing has been
  selected."
  []
...
(defn connected-opts
  ...
  `skein.core.client/call-world`. It carries the explicit state-dir supplied by
...
;;
;; The same register/replace/unregister verbs the `skein.api.*.alpha` tier
...
(defn register-op!
  ...
  Runtime-implied twin of `skein.api.weaver.alpha/register-op!`, which owns the
...
(defn replace-op!
  ...
  Runtime-implied twin of `skein.api.weaver.alpha/replace-op!`: loud when the
  name is absent, and the recorded intent is what carries the shadow across
  `skein.api.runtime.alpha/refresh!`."
  ([op-name fn-sym]
...
(defn unregister-op!
  ...
  Runtime-implied twin of `skein.api.weaver.alpha/unregister-op!`: retracting a
...
(defn register-query!
  ...
  Runtime-implied twin of `skein.api.graph.alpha/register-query!`: re-registering
...
(defn replace-query!
  ...
  Runtime-implied twin of `skein.api.graph.alpha/replace-query!`. Queries are
...
(defn unregister-query!
  ...
  Runtime-implied twin of `skein.api.graph.alpha/unregister-query!`."
  [query-name]
...
(defn register-pattern!
  ...
  Runtime-implied twin of `skein.api.patterns.alpha/register-pattern!`; the doc
...
(defn replace-pattern!
  ...
  Runtime-implied twin of `skein.api.patterns.alpha/replace-pattern!`. Patterns
...
(defn unregister-pattern!
  ...
  Runtime-implied twin of `skein.api.patterns.alpha/unregister-pattern!`."
  [pattern-name]
...
(defn register-hook!
  ...
  Runtime-implied twin of `skein.api.hooks.alpha/register-hook!`; `opts` may
...
(defn replace-hook!
  ...
  Runtime-implied twin of `skein.api.hooks.alpha/replace-hook!`. Hooks bind their
...
(defn unregister-hook!
  ...
  Runtime-implied twin of `skein.api.hooks.alpha/unregister-hook!`."
  [key]
...
(defn register-handler!
  ...
  Runtime-implied twin of `skein.api.events.alpha/register-handler!`."
  ([key types fn-sym]
...
(defn replace-handler!
  ...
  Runtime-implied twin of `skein.api.events.alpha/replace-handler!`. Handlers
...
(defn unregister-handler!
  ...
  Runtime-implied twin of `skein.api.events.alpha/unregister-handler!`."
  [key]
  ...
  (or (current/runtime-or-nil)
      (throw (ex-info (format-alpha/reflow
                       "|Burn-tombstone recovery runs inside the live weaver JVM. No
                         ...
                         |and rerun.")
                      {:helper verb :code :skein.repl/no-in-process-runtime}))))
...
;; in `skein.repl`. Landing a session in the implementation namespace makes the
;; user's own `def`s shadow the machinery they are calling, and it reads as if
;; the helpers were REPL-only rather than ordinary functions. `skein.repl` is
...
(defn- session-ns
  "Return the neutral session namespace, with `skein.repl` required and aliased."
  []
    ...
    (with-open [_ ^java.io.Closeable conn]
      (let [responses (eval-remote-responses! session
                                              {:ns (str session-ns-name)
                                               :code (str "(skein.repl/eval-source-forms! "
                                                          (pr-str source) ")")})]
