...
Use `defworkflow` in module source for a workflow that should survive refresh and restart. The direct
`register-workflow!` and `register-executor!` functions are the explicit-runtime seam for trusted code
and tests, and the `workflow` CLI reads the effective registry. Direct registration is runtime-lifetime
state; `unregister-workflow!` removes only the direct entry and never touches the Var or a module-owned
declaration. If a module should stop publishing a workflow, omit its `defworkflow` form and refresh.
