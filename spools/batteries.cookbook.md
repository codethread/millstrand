...
Honest source: the `update`/BAT-C6/BAT-C20 contract in [`batteries.md`](./batteries.md), the `update-typed-attributes-merge-patch` test in `test/clojure/millstrand/spools/batteries_test.clj`, and the core merge-patch nil deletion in `millstrand.core.db/update-strand!`.
...
strand list --query run-active --param run-id=spool-cookbooks
...
Honest source: the `await` op and its manual-clock band tests in `test/clojure/millstrand/spools/batteries_test.clj`.
