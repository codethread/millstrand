(ns millstrand.spools.workflow
  ...
  (:require [clojure.spec.alpha :as s]
            ...
            [millstrand.spools.workflow.internal.compile :as cmp]
            [millstrand.spools.workflow.internal.definitions :as defs]
            [millstrand.spools.workflow.internal.discovery :as discovery]
            [millstrand.spools.workflow.internal.guard :as guard]
            [millstrand.spools.workflow.internal.query :as query]
            [millstrand.spools.workflow.internal.registry :as registry]
            [millstrand.spools.workflow.internal.routing :as routing]
            [millstrand.spools.workflow.internal.runs :as runs]
  ...
  ([run-id opts]
   (let [rt (current/runtime)]
     ...
     (when (contains? opts :attributes)
       (require-valid! :millstrand.spools.workflow.request/attributes (:attributes opts)
                       "Invalid workflow complete attributes"))
     (let [context (when (contains? opts :context)
                     (let [normalized
                           (-> (require-valid! :millstrand.spools.workflow.values/params
                                               (:context opts)
                                               "Invalid workflow complete context")
                               cmp/default-context)]
                       (require-valid! :millstrand.spools.workflow.request/context
                                       normalized
                                       "Invalid workflow complete context")))]
       (guard/with-run!
         rt run-id
...
(s/def ::executor-entry
  :millstrand.spools.workflow.internal.registry/executor-entry)
...
(defn register-executor!
  ...
  (cond
    ...
    (map? pred) (do (when-not (s/valid? :millstrand.spools.workflow.internal.registry/executor-decl pred)
                      (fail! "Executor declaration map must carry :stalled? (and optionally :request-spec)"
                             {:waiter waiter :pred pred
                              :explain (s/explain-str
                                        :millstrand.spools.workflow.internal.registry/executor-decl pred)}))
...
(defn seed-workflow-vocab!
  ...
  (vocab/declare!
   ...
   {:kind :attr-namespace
    :name "workflow"
    :owner :millstrand/spools-workflow
...
(runtime/collect-kind!
 :millstrand.spools.workflow.internal.registry/registry
 {:id definition-kind
  :entry-spec :millstrand.spools.workflow.internal.registry/definition-symbol
  :binding-moment :route-transition
  :candidate-validator
  'millstrand.spools.workflow.internal.definitions/validate-candidates!})

(runtime/collect-kind!
 :millstrand.spools.workflow.internal.registry/registry
 {:id executor-kind
  :entry-spec :millstrand.spools.workflow.internal.registry/executor-entry
...
(lifecycle/defseed workflow-vocabulary
  ...
  {:apply 'millstrand.spools.workflow/seed-workflow-vocab!})

;; --- input contract specs -------------------------------------------------

(defn- non-blank-string?
  ;; A peer of internal.util/non-blank-string?, kept in this namespace so the
  ;; spec predicates below resolve their symbol to `millstrand.spools.workflow`,
...
(s/def :millstrand.spools.workflow.values/params (s/map-of keyword? any?))
(s/def ::count pos-int?)
(s/def ::chain boolean?)
...
(s/def ::call (s/keys :req-un [::id ::procedure]
                      :opt-un [::title :millstrand.spools.workflow.values/params
...
(s/def ::revise (s/keys :req-un [:millstrand.spools.workflow.values/params]))
(s/def ::choice
  (s/or :key ::id-ref
...
(s/def :millstrand.spools.workflow.request/run-id non-blank-string?)
(s/def :millstrand.spools.workflow.request/workflow ::registry-name)
(s/def :millstrand.spools.workflow.request/params :millstrand.spools.workflow.values/params)
(s/def :millstrand.spools.workflow.request/step non-blank-string?)
(s/def :millstrand.spools.workflow.request/by non-blank-string?)
...
(s/def :millstrand.spools.workflow.request/attributes (s/map-of non-blank-string? any?))
(s/def ::defer-request
  (s/keys :req-un [:millstrand.spools.workflow.request/run-id
                   :millstrand.spools.workflow.request/workflow]
          :opt-un [:millstrand.spools.workflow.request/params
                   :millstrand.spools.workflow.request/step
                   :millstrand.spools.workflow.request/by]))

;; --- discovery request and projection shapes ------------------------------

(s/def :millstrand.spools.workflow.request/entrypoint ::entrypoint)
(s/def ::list-request
  (s/keys :opt-un [:millstrand.spools.workflow.request/entrypoint]))
(s/def ::show-request (s/keys :req-un [:millstrand.spools.workflow.request/workflow]))
...
(s/def :millstrand.spools.workflow.view/name non-blank-string?)
(s/def :millstrand.spools.workflow.view/doc non-blank-string?)
(s/def :millstrand.spools.workflow.view/entrypoints
  (s/coll-of (into #{} (map name) defs/entrypoint-order) :kind vector?))
(s/def :millstrand.spools.workflow.view/definition non-blank-string?)
(s/def :millstrand.spools.workflow.view/step non-blank-string?)
...
(s/def ::catalog-fields
  (s/keys :req-un [:millstrand.spools.workflow.view/name
                   :millstrand.spools.workflow.view/doc
                   :millstrand.spools.workflow.view/entrypoints
                   :millstrand.spools.workflow.view/definition]))
(s/def ::catalog-item
...
(s/def :millstrand.spools.workflow.view.params/kind #{"spec" "none"})
(s/def :millstrand.spools.workflow.view.params/spec non-blank-string?)
(s/def :millstrand.spools.workflow.view.params/spec-form
...
(s/def :millstrand.spools.workflow.view.params/spec-forms
  (s/coll-of :millstrand.spools.workflow.view.params/spec-form :kind vector?))
(s/def :millstrand.spools.workflow.view.params/defaults (s/map-of keyword? any?))
(s/def :millstrand.spools.workflow.view.params/contract (s/map-of string? any?))
(s/def :millstrand.spools.workflow.view.params/template any?)
(s/def :millstrand.spools.workflow.view.params/example ::example)
(s/def :millstrand.spools.workflow.view/params
  (s/keys :req-un [:millstrand.spools.workflow.view.params/kind]
          :opt-un [:millstrand.spools.workflow.view.params/spec
                   :millstrand.spools.workflow.view.params/spec-forms
                   :millstrand.spools.workflow.view.params/contract
                   :millstrand.spools.workflow.view.params/template
                   :millstrand.spools.workflow.view.params/defaults
                   :millstrand.spools.workflow.view.params/example]))
...
(s/def :millstrand.spools.workflow.view.declared/kind #{"static"})
(s/def :millstrand.spools.workflow.view.declared/each non-blank-string?)
(s/def :millstrand.spools.workflow.view.declared/waiter non-blank-string?)
(s/def :millstrand.spools.workflow.view.declared/procedure non-blank-string?)
...
(s/def :millstrand.spools.workflow.view.call/kind
  #{"registered" "symbol" "var" "inline"})
(s/def :millstrand.spools.workflow.view.declared/defer non-blank-string?)
(s/def :millstrand.spools.workflow.view.declared/entrypoint #{"call"})
(s/def :millstrand.spools.workflow.view.declared/names
  (s/coll-of non-blank-string? :kind vector?))
(s/def :millstrand.spools.workflow.view.declared/entry
  :millstrand.spools.workflow.view.declared/names)
(s/def :millstrand.spools.workflow.view.declared/choices
  :millstrand.spools.workflow.view.declared/names)
(s/def :millstrand.spools.workflow.view.declared/workflows
  :millstrand.spools.workflow.view.declared/names)
(s/def :millstrand.spools.workflow.view.declared/routes
  :millstrand.spools.workflow.view.declared/names)
(s/def :millstrand.spools.workflow.view.declared/loop
  (s/keys :req-un [:millstrand.spools.workflow.view/step]
          :opt-un [:millstrand.spools.workflow.view.declared/each ::count ::chain]))
(s/def :millstrand.spools.workflow.view.declared/loops
  (s/coll-of :millstrand.spools.workflow.view.declared/loop :kind vector?))
(s/def :millstrand.spools.workflow.view.declared/gate
  (s/keys :req-un [:millstrand.spools.workflow.view/step
                   :millstrand.spools.workflow.view.declared/waiter]))
(s/def :millstrand.spools.workflow.view.declared/gates
  (s/coll-of :millstrand.spools.workflow.view.declared/gate :kind vector?))
(s/def :millstrand.spools.workflow.view.declared/checkpoint
  (s/keys :req-un [:millstrand.spools.workflow.view/step
                   :millstrand.spools.workflow.view.declared/choices]))
(s/def :millstrand.spools.workflow.view.declared/checkpoints
  (s/coll-of :millstrand.spools.workflow.view.declared/checkpoint :kind vector?))
(s/def :millstrand.spools.workflow.view.declared/call
  (s/keys :req-un [:millstrand.spools.workflow.view/step
                   :millstrand.spools.workflow.view.declared/procedure
                   :millstrand.spools.workflow.view.call/kind]))
(s/def :millstrand.spools.workflow.view.declared/calls
  (s/coll-of :millstrand.spools.workflow.view.declared/call :kind vector?))
(s/def :millstrand.spools.workflow.view.declared/defer-point
  (s/keys :req-un [:millstrand.spools.workflow.view/step
                   :millstrand.spools.workflow.view.declared/defer
                   :millstrand.spools.workflow.view.declared/workflows
                   :millstrand.spools.workflow.view.declared/entrypoint]))
(s/def :millstrand.spools.workflow.view.declared/defers
  (s/coll-of :millstrand.spools.workflow.view.declared/defer-point :kind vector?))
(s/def :millstrand.spools.workflow.view/declared
  (s/keys :req-un [:millstrand.spools.workflow.view.declared/kind]
          :opt-un [:millstrand.spools.workflow.view.declared/entry
                   :millstrand.spools.workflow.view.declared/loops
                   :millstrand.spools.workflow.view.declared/gates
                   :millstrand.spools.workflow.view.declared/checkpoints
                   :millstrand.spools.workflow.view.declared/calls
                   :millstrand.spools.workflow.view.declared/defers
                   :millstrand.spools.workflow.view.declared/routes]))
...
(s/def ::definition-view
  (s/and (s/merge ::catalog-fields
                  (s/keys :req-un [:millstrand.spools.workflow.view/params
                                   :millstrand.spools.workflow.view/declared]))
...
(s/def :millstrand.spools.workflow.view.executor/waiter non-blank-string?)
(s/def :millstrand.spools.workflow.view.executor/stall-predicate
  (s/nilable non-blank-string?))
(s/def :millstrand.spools.workflow.view.executor/request
  (s/keys :req-un [:millstrand.spools.workflow.view.params/spec
                   :millstrand.spools.workflow.view.params/spec-forms
                   :millstrand.spools.workflow.view.params/contract
                   :millstrand.spools.workflow.view.params/template]))
(s/def ::executor-view
  (s/and (s/keys :req-un [:millstrand.spools.workflow.view.executor/waiter
                          :millstrand.spools.workflow.view.executor/stall-predicate]
                 :opt-un [:millstrand.spools.workflow.view.executor/request])
         #(every? #{:waiter :stall-predicate :request} (keys %))))

;; --- run lifecycle request and result shapes ------------------------------

(s/def :millstrand.spools.workflow.request/choice non-blank-string?)
(s/def :millstrand.spools.workflow.request/input :millstrand.spools.workflow.values/params)
(s/def :millstrand.spools.workflow.request/context
  (s/and :millstrand.spools.workflow.values/params json-safe-context?))
(s/def :millstrand.spools.workflow.request/timeout-secs ::timeout-secs)

(s/def :millstrand.spools.workflow.advance/choice
  #(or (keyword? %) (non-blank-string? %)))
(s/def ::advance-opts
  (s/keys :opt-un [:millstrand.spools.workflow.advance/choice
                   :millstrand.spools.workflow.request/input
                   :millstrand.spools.workflow.request/step
                   :millstrand.spools.workflow.request/by
                   :millstrand.spools.workflow.request/attributes]))
...
(s/def ::choices-request
  (s/keys :req-un [:millstrand.spools.workflow.request/run-id]
          :opt-un [:millstrand.spools.workflow.request/step]))
(s/def ::complete-request
  (s/keys :req-un [:millstrand.spools.workflow.request/run-id]
          :opt-un [:millstrand.spools.workflow.request/step
                   :millstrand.spools.workflow.request/by
                   :millstrand.spools.workflow.request/attributes
                   :millstrand.spools.workflow.request/context]))
(s/def ::choose-request
  (s/keys :req-un [:millstrand.spools.workflow.request/run-id
                   :millstrand.spools.workflow.request/choice]
          :opt-un [:millstrand.spools.workflow.request/input
                   :millstrand.spools.workflow.request/step
                   :millstrand.spools.workflow.request/by]))
(s/def ::next-request
  (s/keys :req-un [:millstrand.spools.workflow.request/run-id]
          :opt-un [:millstrand.spools.workflow.request/choice
                   :millstrand.spools.workflow.request/input
                   :millstrand.spools.workflow.request/step
                   :millstrand.spools.workflow.request/by]))
(s/def ::await-request
  (s/keys :req-un [:millstrand.spools.workflow.request/run-id]
          :opt-un [:millstrand.spools.workflow.request/timeout-secs]))
...
(s/def :millstrand.spools.workflow.view/id non-blank-string?)
(s/def :millstrand.spools.workflow.view/title string?)
(s/def :millstrand.spools.workflow.view/state ::state)
...
(s/def :millstrand.spools.workflow.view/role #{"step" "checkpoint" "defer"})
(s/def :millstrand.spools.workflow.view/run-id non-blank-string?)
(s/def :millstrand.spools.workflow.view/operation non-blank-string?)
(s/def :millstrand.spools.workflow.view/done boolean?)
(s/def :millstrand.spools.workflow.view/gate non-blank-string?)
(s/def :millstrand.spools.workflow.view/defer non-blank-string?)
(s/def :millstrand.spools.workflow.view/workflows (s/coll-of non-blank-string?))
(s/def :millstrand.spools.workflow.view/choices (s/coll-of non-blank-string?))

(s/def ::root-view
  (s/keys :req-un [:millstrand.spools.workflow.view/id
                   :millstrand.spools.workflow.view/title
                   :millstrand.spools.workflow.view/state]))
...
(s/def ::ready-fields
  (s/keys :req-un [:millstrand.spools.workflow.view/id
                   :millstrand.spools.workflow.view/role
                   :millstrand.spools.workflow.view/title]))
...
(defmethod ready-item-branch :checkpoint [_]
  (s/merge ::ready-fields (s/keys :req-un [:millstrand.spools.workflow.view/choices])))

(defmethod ready-item-branch :defer [_]
  (s/merge ::ready-fields (s/keys :req-un [:millstrand.spools.workflow.view/defer
                                           :millstrand.spools.workflow.view/workflows])))

(defmethod ready-item-branch :gate [_]
  (s/merge ::ready-fields (s/keys :req-un [:millstrand.spools.workflow.view/gate])))
...
(s/def :millstrand.spools.workflow.view/root ::root-view)
(s/def :millstrand.spools.workflow.view/ready (s/coll-of ::ready-item :kind vector?))
(s/def ::run-result
  (s/keys :req-un [:millstrand.spools.workflow.view/operation
                   :millstrand.spools.workflow.view/run-id
                   :millstrand.spools.workflow.view/root
                   :millstrand.spools.workflow.view/ready
                   :millstrand.spools.workflow.view/done]))
...
(s/def :millstrand.spools.workflow.view/reason
  #{:done :checkpoint :defer :step :gate :stalled :waiting :timeout})
(s/def :millstrand.spools.workflow.view/detail map?)
(s/def ::attention-result
  (s/keys :req-un [:millstrand.spools.workflow.view/operation
                   :millstrand.spools.workflow.view/run-id
                   :millstrand.spools.workflow.view/reason
                   :millstrand.spools.workflow.view/ready
                   :millstrand.spools.workflow.view/done]
          :opt-un [:millstrand.spools.workflow.view/detail]))
...
(s/def :millstrand.spools.workflow.view.choices/input-spec
...
(s/def :millstrand.spools.workflow.view.choices/detail
  (s/and (s/map-of #{"label" "description" "next" "revise" "input-spec"} any?)
         ...
         #(every? (partial s/valid? :millstrand.spools.workflow.view.choices/input-spec)
                  (vals (select-keys % ["input-spec"])))))
(s/def :millstrand.spools.workflow.view.choices/choices
  (s/map-of non-blank-string? :millstrand.spools.workflow.view.choices/detail))
(s/def ::choices-result
  (s/keys :req-un [:millstrand.spools.workflow.view/operation
                   :millstrand.spools.workflow.view/run-id
                   :millstrand.spools.workflow.view.choices/choices]))
...
(defn- explain-workflow []
  {:topic :workflow
   ...
   :builders {'workflow 'millstrand.spools.workflow/workflow
              'defworkflow 'millstrand.spools.workflow/defworkflow
              'step 'millstrand.spools.workflow/step
              'gate 'millstrand.spools.workflow/gate
              'checkpoint 'millstrand.spools.workflow/checkpoint
              'call 'millstrand.spools.workflow/call
              'defer 'millstrand.spools.workflow/defer
              'bind-defers 'millstrand.spools.workflow/bind-defers}
