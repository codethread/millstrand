(ns millstrand.spools.workflow.internal.discovery
  ...
  *declares* — its entry items, loops, gates, checkpoints, calls, defers, and the
  ...
  (:require [millstrand.spools.workflow.internal.definitions :as defs]
            [millstrand.spools.workflow.internal.registry :as registry]
            [millstrand.spools.workflow.internal.specs :as specs]
            [millstrand.spools.workflow.internal.util :as util :refer [require-shape!]]))
...
(defn- catalog-item
  ...
  [resolved]
  (require-shape! :millstrand.spools.workflow/catalog-item
...
(defn catalog
  ...
  [rt request]
  (require-shape! :millstrand.spools.workflow/list-request request
...
(defn definition-view
  ...
  repair it."
  [rt name]
  (require-shape! :millstrand.spools.workflow/show-request {:workflow name}
  ...
  (let [resolved (defs/resolve-registered rt name)
    ...
    (require-shape! :millstrand.spools.workflow/definition-view
...
(defn executor-catalog
  ...
  (mapv (fn [[waiter {:keys [stalled? request-spec]}]]
          (require-shape! :millstrand.spools.workflow/executor-view
                          (cond-> {:waiter waiter
