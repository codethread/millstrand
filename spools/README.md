...
Every spool loads through one convention: an ordinary coordinate in `.millstrand/deps.edn` or `deps.local.edn`, and an explicit stable `runtime/module!` declaration in `init.clj` or `init.local.clj`. Dependency presence never activates code. A running Weaver has one immutable tools.deps basis; changing dependencies requires generation replacement, while workspace-relative `:file` modules remain live. Full refresh collects selected authoring declarations, publishes the module's owner-complete contribution, and runs lifecycle declarations. [Customising your workspace](../docs/spools/customisation.md) is the operational walkthrough; [Migrating from spool manifests](../docs/spools/deps-migration.md) maps the removed surface.
...
The batteries and unsafe-text-search spools remain in this checkout under `spools/<name>/src`, off the production weaver classpath. Their `{:millstrand/source-root "spools/<name>"}` coordinates resolve against the mill-selected Millstrand checkout. Workflow, Chime, Cron, Kanban, and the gate executors are external Millhouse roots, pinned by this workspace at [`3af5786f`](https://github.com/codethread/millhouse.spool/tree/f1cdda3b46706b186f547251d285791be650d232). Their contracts live in that repository. For publishing a spool by git coordinate, SHA-pinned approval, README dependency/activation snippets, Maven-only spool-root dependencies, and local development overrides, see [Writing shared spools](../docs/spools/writing-shared-spools.md#publishing-a-shared-spool-with-git-distribution).
