...
Cron is kept as a quality-gated example under [`examples/cron`](../examples/cron). It is not approved or activated by this repository's default workspace. Its contract, cookbook, API reference, source, and example job remain available for workspaces that opt in.
