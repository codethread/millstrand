...
| `ct.spools.devflow` | git, sha-pinned (see below) | [devflow.md](https://github.com/codethread/devflow.spool/blob/6810b5259dff23e291a2bf46071c0680cf9731b4/devflow.md) | — | Reference devflow lifecycle built on the workflow engine: intake → proposal → decomposition into implementation cards, with HITL checkpoints. Card authoring is a `workflow/defer`, so a workspace picks the target; this repo binds its kanban board in `.skein/devflow_targets.clj`. |
