...
A spool activates by declaring a module: `(runtime/module! runtime key opts)` names a source target, and the coordinator collects its contribution, publishes it through the owner-partitioned registry kernel, and runs its `:reconcile` for live effects. That is how production activates every spool today (`.millstrand/init.clj`).

`install!` is the predecessor: an imperative function per spool that registered everything by direct mutation. At the time of this decision it survives only where nobody looked — as test fixtures. The census (epic `waq0l`, investigation 2026-07-23): 6 in-tree definitions (batteries, workflow, executors.shell, chime, cron, unsafe-text-search), 21 call sites all under `test/`, roughly 76 fixture executions per suite run, zero production callers. Three sibling repos carry the same pattern (devflow.spool, kanban.spool, dresser.spool; agent-harness.spool already had its installers removed by OLR F17).
...
**Superseded in part on 2026-07-24 by [ADR-004.P3](0004-def-spool-convention.md):** the explicit-`:contribute` requirement and its declaration-time refusal below are historical. Image evaluation now resolves contribution from the loaded target namespace's public `spool` var, with a Phase A explicit key still accepted; when neither supplies contribution, activation fails through the per-module evaluation channel. The bullets below preserve the original decision and call out the current behavior where it differs.

- Valid only with an `:ns` target — combined with `:file` it is refused at declaration time. The declared namespace must already be loaded in the JVM image; otherwise the module fails through the evaluation channel (ADR-004.P3, SPEC-004.C46).
- **Historical, superseded by ADR-004.P3:** required an explicit `:contribute` symbol because no source evaluation or authoring-form collection happens, and refused a `:load :image` declaration without one at declaration time. The current convention resolves the loaded namespace's public `spool` var, retains the Phase A explicit fallback, and reports the absence of both through module evaluation.
- Every refusal or evaluation failure carries the failing declaration's module key and offending value plus the allowed alternatives — which target kinds are accepted, that an image namespace must be loaded first, and that contribution must come from its public `spool` var or the Phase A explicit fallback — per TEN-003's message discipline. A failure a caller cannot act on from its own text is a defect in the core feature, not an acceptable stub.
...
The remaining epic features convert every `install!` fixture to module activation. Except where a bullet records a later supersession, these conventions are inherited from the gate riders (note `s9dp0`, as amended by `mtl40`) and the OLR peer conversions; they are binding for the conversions and the default for future test design. Current test guidance lives in [`docs/spools/testing.md`](../../docs/spools/testing.md), and [ADR-004](0004-def-spool-convention.md) governs the public `def spool` declaration that replaced the exported module datum.
...
- If `{:load :image}`'s explicit-`:contribute` requirement becomes the common friction — many spools authoring contribution fns solely to satisfy image mode — revisit whether collection can honestly serve image-owned namespaces. Do not revisit by relaxing the loud failures. _Resolved 2026-07-24 by [ADR-004](0004-def-spool-convention.md).P3:_ image evaluation resolves `<ns>/spool` from the loaded image, retains a Phase A explicit `:contribute` fallback, and reports a loud evaluation-time `:failed` outcome when neither supplies contribution.
