...
| **release** | The coordinator release workflow: bump `VERSION` and the changelog, run the full quality contract, pin the Homebrew formula to the release commit, verify binary identity, then obtain human sign-off before tagging and atomically pushing `main` plus the tag. | Deploy, land, publish bare |
