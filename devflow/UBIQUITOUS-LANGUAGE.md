...
| **Weaver generation** | One isolated weaver process lifetime. Its generation basis and classloader are minted at boot and never swapped while the process runs. In a JVM pool, use **member runtime generation** for this identity and reserve **pool host generation** for the shared process lifetime. | Schema generation, version, restart, session |
