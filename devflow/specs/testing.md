...
- **SPEC-006.C3 (weaver world):** Behavior that depends on workspace startup files, the resolved tools.deps basis, module publication, the generation classloader, runtime transports, storage, events, scheduling, reload, or basis-change handling uses `millstrand.test.alpha` to create a real disposable weaver world. The helper contract remains owned by `SPEC-003.C28`–`C31`; runtime publication and isolation remain owned by `SPEC-004.C8a`. A dependency fixture supplies its own `deps.edn`, optional `deps.local.edn`, `init.clj`, and optional `init.local.clj`.
...
- **SPEC-006.C6:** Direct `require` resolves through the author's test JVM classpath. Forms sent through `millstrand.test.alpha/repl!` evaluate through the disposable Weaver's real nREPL transport and generation classloader. Code is visible only when its coordinate belongs to the resolved selected-workspace basis and its module is explicitly activated through `init.clj` or `init.local.clj`. Fixtures disable Clojure user configuration when constructing or inspecting a Weaver basis.
- **SPEC-006.C7:** An integration fixture that claims dependency loading supplies ordinary tools.deps coordinates and explicit activation files in a disposable workspace. A local-replacement fixture puts the shared coordinate in `deps.edn` and its replacement in `deps.local.edn`. It uses fresh namespaces and paths; a classpath-only `require` is not evidence.
- **SPEC-006.C8:** `millstrand.test.alpha/spool-checkout-root` bridges a file-backed source resource on the test classpath to the checkout root used as an ordinary tools.deps `:local/root`. It does not approve or activate a dependency and does not write dependency data.
...
- **SPEC-006.C13:** Tests activate modules through the same declaration, contribution, lifecycle, and refresh contracts used by production. `millstrand.test.alpha/activate-module!` is the narrow classpath-visible bare-runtime helper for direct authoring tests; it accepts only `:after` and `:load :image` and does not prove dependency-basis or startup-file loading. Tests making those claims use a generated Weaver world with dependency and activation files. A test-only registration back door is not part of the consumer surface.
