...
This spec defines how downstream projects may test code against Skein and how spool authors may prove their code in a real weaver. It owns the supported testing tiers and the boundary between shipped author-side helpers and Skein's repository-only fixtures. Detailed API and runtime behavior remains with `SPEC-003`, `SPEC-004`, and `SPEC-005`.
...
- **SPEC-006.G3:** Keep test runtimes isolated from ambient and user-owned Skein worlds.
...
- **SPEC-006.NG1:** Skein does not prescribe a downstream test runner or add an assertion DSL.
...
- **SPEC-006.NG3:** Skein's own test namespaces, directories, runner groups, queue discipline, and repository-only fixtures are not consumer contracts.
...
- **SPEC-006.C5:** Until Skein has a published artifact contract, an external project selects a Skein checkout as a tools.deps `:local/root` test dependency. The dependency belongs on the test classpath, not the spool's production source path. CI selects a reviewed release tag or full commit SHA; Skein defines no floating latest test target.
- **SPEC-006.C6:** Direct `require` resolves through the author's test JVM classpath. Forms sent through `skein.test.alpha/repl!` evaluate through the disposable weaver's real nREPL transport and runtime binding. Spool code is visible to that weaver only through its approved-root and module-loading model.
...
- **SPEC-006.C8:** `skein.test.alpha/spool-checkout-root` is the supported bridge from a file-backed source resource on the test classpath to the local checkout root a generated `spools.edn` may approve. Its resolution and fail-loud behavior remain owned by `SPEC-003.C32`.

## SPEC-006.P6 World and lifecycle contract

- **SPEC-006.C9:** `skein.test.alpha` is the shipped author-side testing surface and follows the alpha compatibility discipline in `SPEC-005.C2`. `skein.spools.test-support` and every other helper under Skein's own test roots are internal repository fixtures.
- **SPEC-006.C10:** The weaver-world tier is the isolated generated-world composition specified by `SPEC-003.C29`–`C31`, using unpublished runtimes under `SPEC-004.C8a`. Skein does not offer a consumer testing tier that operates on the author's normal workspace.
...
- **Rationale:** A broad harness would duplicate product APIs and tie consumers to Skein's internal suite.
...
- **SPEC-006.C15:** A change to downstream testing tiers, the shipped `skein.test.alpha` surface, world isolation, or what a tier proves updates this spec and the practical guide. Internal test moves, runner changes, repository lint rules, and internal fixture refactors do not.
