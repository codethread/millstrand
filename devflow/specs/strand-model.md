...
The canonical omission descriptor contract is `:millstrand.core.specs/omitted-attribute-descriptor`. Emitters must produce values conforming to it, and consumers must discriminate omission through it. Its typed map is `{:millstrand/omitted true :bytes N}`, where `:millstrand/omitted` is literally `true` and `:bytes` is the non-negative JSON-encoded UTF-8 byte length of the omitted value. It is storage-neutral: it means "omitted from this read surface", not "stored elsewhere". No storage mechanism is claimed or implied.
...
Storage declares the `notes` relation acyclic but does not impose note shape, lifecycle, or cardinality. The blessed notes API and Batteries operations create a closed note strand with `note/text` (content), `note/at` (write time), optional `identity/by-identity` and `note/round`, and an outgoing `notes` edge to its target (note `--notes--> target`). `identity/by-identity` is a supplied non-blank friendly actor string; writing it does not require an Identity spool, registry lookup, or identity edge. In that convention the edge is the sole target linkage: no target-pointing attribute duplicates it. The primitive owns `note/text`, `note/at`, and `note/round`; writers must not supply those keys as decorations, whether string- or keyword-keyed. Other decorating attributes remain mutable unless separately declared immutable. Historic `note/by` values remain uninterpreted old data; Millstrand does not invent attribution for them.
...
Edge entries are ordered `:upsert` or `:remove` operations addressed by valid local refs. Both endpoints must resolve to existing or batch-created strands, the relation must be a valid name, and burned refs are invalid endpoints. An upsert carries `:from`, `:to`, `:type`, and optional JSON-object `:attributes`; it creates the edge or replaces the attributes of the matching `(from, to, type)` edge, and may use refs created by the batch. It rejects self-edges and writes that violate declared relation acyclicity. A remove carries exactly `:op`, `:from`, `:to`, and `:type`; both endpoints must be top-level bound refs. It deletes that exact edge identity, regardless of attributes. Removing an absent edge is a strict failure with ex-data `{:from <ref> :to <ref> :from-id <durable-id> :to-id <durable-id> :type <relation>}` and rolls back the batch. Duplicate refs and invalid bindings or references fail before mutation. Edge operations execute in submitted order, so a remove followed by an upsert of the same identity has deterministic replacement semantics. Omitted edges are unchanged.

The public `millstrand.api.batch.alpha/apply!` result contains the final ref table and normalized summaries of created, updated, and burned strands plus ordered edge transitions. Each transition has exactly `:op`, the submitted `:from`, `:to`, and `:type`, and `:before`/`:after` values containing either `nil` or a normalized edge row with exactly `:from_strand_id`, `:to_strand_id`, `:edge_type`, and `:attributes`; the first three use durable ids and relation text, and `:attributes` is a decoded map. Upserts report the previous and written rows; removes report the removed row and `nil`. The returned `:edges`, pre-commit hook `:batch/edge-ops`, and `:batch/applied` event `:batch/edges` are the same ordered transition vector.
