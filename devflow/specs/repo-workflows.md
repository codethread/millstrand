...
- **SPEC-006.C4:** The Ralph readiness gate depends on every feature review checkpoint. When ready, it proves the reviewed ids exactly match the epic's direct features, revalidates every condition in SPEC-006.C1 and SPEC-006.C2, then applies the `ralph` label through kanban's atomic label operation. Any validation mismatch fails before labeling the epic. The gate rereads the persisted label; an unexpected value is removed before the gate fails with expected and observed values.
