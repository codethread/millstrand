...
- **SPEC-006.C2:** Before review, the coordinator decomposes the epic into direct feature cards and gives every feature a non-empty task-strand DAG. The workflow records the complete, non-empty set of unique feature ids at a typed checkpoint.
...
- **SPEC-006.C4:** The Ralph readiness gate depends on every feature review checkpoint. When ready, it revalidates that each reviewed feature is a direct child of the epic and has at least one task, applies the `ralph` label, and verifies the persisted label postcondition. Any mismatch fails without labeling the epic.
