...
> This RFC merges the two same-day drafts `RFC-Saf-001` (spool authoring forms) and `RFC-Laf-001` (lifecycle authoring forms), which proposed the two halves of one change as separate documents and spent much of their text deferring to each other. Git history holds the originals; references to either id resolve here.
...
During the migration window (P15.S2), unmigrated spools still carry their old `spool` var, but no released contract teaches or floors onto that state, and the landed final grammar has none.
...
- **RFC-Auf-001.S2 — Migration.** Convert every in-tree spool, the repo `.skein` config, examples, and tests to authoring forms; coordinate sibling spool releases and pin bumps so the selected source universe contains no `:contribute` or `:reconcile` declaration. Prove parity before the break: equal normalized contributions, exact removal by omission, and lossless lifecycle behavior for each migrated reconciler. Sibling migrations ship as ordinary marker releases consumed by pin bumps, with `:requires`/`:skein/min` floor raises and `bin/compat-alarm` evidence landing with each release. The interval in which both grammars load is a migration window, not a supported contract: no floor targets it, no doc teaches it, and a module may not declare both collected authoring forms and a `:contribute` entry point at once — already the shipped rule (SPEC-004.C46c), which the window keeps.
