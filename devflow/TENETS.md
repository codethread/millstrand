...
- **TEN-007**: Storage complexity is the core's burden; the attribute map is the contract.
  - A strand *has* an attribute map. How those attributes are physically stored is an implementation detail owned entirely by `millstrand.core.*`. No consumer above `millstrand.core.*` — `millstrand.api.*`, spool authors, the CLI JSON wire format, the query language, events, or views — may depend on the physical shape of attribute storage.
