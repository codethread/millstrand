...
- `26-07-03__agents-spool` — shipped the `skein.spools.agents` spool owning the full `strand agent` delegation surface (delegate/retry/status/plan) over the pure shuttle engine, with repo `.millstrand/policy/config.clj` shrunk to genuine workspace tuning.
...
- `26-07-05__review-fanout` — shipped the declarative reviewer roster: spec-defined roster data (`.millstrand/agents/reviewers.clj`), `defroster!`/`rosters`, `agent review --roster` fan-out with per-reviewer contracts/scopes, review-pass tags, and the public `roster-review-specs` composition seam for workflow authors.
