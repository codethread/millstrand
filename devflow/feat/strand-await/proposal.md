...
- **PROP-Sawt-001.S2:** One flat result map for both outcomes: `operation`, `query`, `reason` (satisfied | timeout), `count`, `min_count`, `max_count`, `elapsed_ms`. Whichever bound was omitted is null in the result; the other fields are always present. `count` is the observed match count from the bounded probe: exact whenever it decides the outcome, clamped at the probe limit otherwise — await reports the band decision, never a progress meter.
