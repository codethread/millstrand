...
- **PROP-Sad-001.S10:** The migration universe is closed to five repositories: Millstrand, Millhouse, agent-harness, devflow, and codethread. Each migrates publishing entry points to bang forms and uses the split forms where it deliberately exposes selectable declarations. Kanban is an explicitly authorized external cutover: its source owner may publish a Guild-free successor and this workspace may pin it. The Millstrand-owned consumer adapter selects only Kanban's core board operations, queries, pattern, and bin; Kanban peering operations and its lifecycle resource are out of scope for the Millstrand surface. A disposable fresh-generation acceptance test must prove the successor pin before any real weaver restart. No Kanban source is changed in this repository, and the adapter does not restore the old publishing meaning of `def<kind>`.
