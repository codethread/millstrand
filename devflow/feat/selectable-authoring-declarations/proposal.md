...
- **PROP-Sad-001.S10:** The migration universe is closed to five repositories: Millstrand, Millhouse, agent-harness, devflow, and codethread. Each migrates publishing entry points to bang forms and uses the split forms where it deliberately exposes selectable declarations. The selected workspace keeps the unchanged Kanban v24 tag at peeled SHA `87f61bc2750e7026f3650235907db25f19b1536e`. The Millstrand-owned consumer adapter selects only ops `kanban` and `kanban-export`; queries `kanban-cards`, `kanban-pending`, and `kanban-epic-pending`; pattern `kanban-batch`; and bin `kanban-dash`. Guild, peering operations, and lifecycle resources are out of scope for the Millstrand surface. A disposable fresh-generation acceptance test proves the selected v24 adapter surface and retained-image replay before any real weaver restart. The historical v24 full repository suite still requires the removed Guild root; that incompatibility remains a gate fact and is not waived by this proposal. No Kanban source is changed in this repository, and the adapter does not restore the old publishing meaning of `def<kind>` or add a compatibility bridge.
...
The approved proposal originally named Kanban v25 at peeled SHA `a6b3a36cd5476ec5c36cd58a7f74bfec6b7e665e`. That target is superseded by the unchanged Kanban v24 tag and peeled SHA `87f61bc2750e7026f3650235907db25f19b1536e` in `PROP-Sad-001.S10`. The v25 target is retained here only as historical traceability; it is not an active pin or acceptance target. The v24 Guild-suite incompatibility remains factual, and this reconciliation changes no acceptance policy or adds any waiver.
