...
- **PROP-Sad-001.S10:** The migration universe is closed to five repositories: Millstrand, Millhouse, agent-harness, devflow, and codethread. Each migrates publishing entry points to bang forms and uses the split forms where it deliberately exposes selectable declarations. Kanban v25 at peeled SHA `a6b3a36cd5476ec5c36cd58a7f74bfec6b7e665e` is the explicitly authorized external exception. The Millstrand-owned consumer adapter selects only ops `kanban` and `kanban-export`; queries `kanban-cards`, `kanban-pending`, and `kanban-epic-pending`; pattern `kanban-batch`; and bin `kanban-dash`. Guild, peering operations, and lifecycle resources are out of scope for the Millstrand surface. A disposable fresh-generation acceptance test must prove the v25 pin before any real weaver restart. No Kanban source is changed in this repository, and the adapter does not restore the old publishing meaning of `def<kind>` or add a compatibility bridge.
