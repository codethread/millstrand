...
- **PROP-Sad-001.S3:** `use-<kind>!` accepts one or more declaration Vars from its own family and contributes them to the current module source. It takes an optional leading use-options map, initially closed to `:override?`. Passing a Var from another family, a Var without authoring metadata, an invalid descriptor, or the same effective key twice in one use form fails loudly at that form. Separate top-level use forms retain the current collector rule from `SPEC-003.C17`: repeating a kind and key replaces the earlier contribution deterministically, and the later selection's override intent is the one retained.
- **PROP-Sad-001.S4:** `def<kind>!` is the permanent define-and-contribute shorthand for workspace-owned declarations and spool entry points. Its definition contract is the same as `def<kind>`; any selection-only option is routed to the generated `use-<kind>!` step rather than stored as library declaration intent.
...
- **PROP-Sad-001.S10:** The migration universe is closed to five repositories: Millstrand, Millhouse, agent-harness, devflow, and codethread. Each migrates publishing entry points to bang forms and uses the split forms where it deliberately exposes selectable declarations. Millstrand's selected workspace keeps the excluded `codethread/kanban` pin unchanged but replaces its direct module declaration with a Millstrand-owned consumer adapter. In a fresh generation, the unchanged Kanban source is compiled against the new inert forms and therefore defines descriptor-bearing Vars; the adapter selects the exact operations, queries, patterns, bins, and lifecycle declarations the workspace uses. A disposable fresh-generation acceptance test must prove the adapter before any real weaver restart. No Kanban source or release is changed, and this adapter does not restore the old publishing meaning of `def<kind>`.
