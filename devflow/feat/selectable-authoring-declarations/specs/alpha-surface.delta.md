...
- **DELTA-Sad-003.CC4:** The coordinated userland migration is limited to Millhouse, agent-harness, devflow, and codethread. Their own contracts and release markers record their corresponding breaks. The selected workspace keeps the unchanged Kanban v24 tag at peeled SHA `87f61bc2750e7026f3650235907db25f19b1536e`. This repository consumes only the selected adapter surface through the Millstrand-owned adapter and fresh-generation constraint in `DELTA-Sad-002.CC7`; Guild, peering operations, and lifecycle resources are absent from that surface. The historical v24 full repository suite still requires the removed Guild root. No compatibility bridge or other compatibility claim is made for an external spool repository.
