...
- **DELTA-Sad-003.CC4:** The coordinated userland migration is limited to Millhouse, agent-harness, devflow, and codethread. Their own contracts and release markers record their corresponding breaks. Kanban is an explicitly authorized external successor pin: its source owner may publish a Guild-free core-only source, while this repository consumes it only through the Millstrand-owned adapter and fresh-generation constraint in `DELTA-Sad-002.CC7`. No other compatibility claim is made for an external spool repository.
