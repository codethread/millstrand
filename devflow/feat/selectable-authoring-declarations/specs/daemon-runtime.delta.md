...
- **DELTA-Sad-002.CC7:** A fresh source-mode generation can load the authorized Guild-free Kanban successor against the new Millstrand macros, then select its core-only descriptor-bearing Vars from the consumer adapter. The successor source and pin are owned by the external Kanban change; this repository does not edit that source or invent its SHA. The adapter selects no peering operation or lifecycle resource. A disposable fresh-generation acceptance proof is required before restart, and its retained adapter record follows CC4 for image replay.
