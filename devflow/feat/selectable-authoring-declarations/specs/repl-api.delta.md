...
- **DELTA-Sad-001.CC2 (replaces the authoring portion of SPEC-003.C17f):** `millstrand.api.lifecycle.alpha` owns `defseed`/`use-seed!`/`defseed!`, `defresource`/`use-resource!`/`defresource!`, and `defreconcile`/`use-reconcile!`/`defreconcile!`. Inert and bang forms use the existing name, docstring, and declaration-option grammar, and both return the installed Var. Lifecycle selection has no override option because effect identity is `[consumer-module effect-id]`; its typed use forms accept only declaration Var references.
