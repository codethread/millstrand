...
Deliver the approved three-form authoring API as one coordinated break: inert `def<kind>`, typed `use-<kind>!`, and define-and-select `def<kind>!`. Change only Millstrand, Millhouse, agent-harness, devflow, and codethread. “Millhouse” includes every root in that repository, including `millhouse.spools/kanban`; it does not include the separately pinned `codethread/kanban` repository. The selected workspace keeps the unchanged Kanban v24 tag at peeled SHA `87f61bc2750e7026f3650235907db25f19b1536e`. Its Millstrand-owned adapter selects only ops `kanban` and `kanban-export`; queries `kanban-cards`, `kanban-pending`, and `kanban-epic-pending`; pattern `kanban-batch`; and bin `kanban-dash`. Guild, peering operations, and lifecycle resources are absent from the Millstrand surface.
...
Outcome: Millstrand ships the complete descriptor, built-in family, Kondo, test, documentation, and built-in/lifecycle source migration as one green commit. Workspace forms owned by external domains remain an explicit cutover inventory until their new APIs are pinned; the running weaver is neither refreshed nor restarted in this mixed interval. A disposable fresh-generation world loads the unchanged Kanban v24 at peeled SHA `87f61bc2750e7026f3650235907db25f19b1536e` through the Millstrand-owned adapter, selects ops `kanban` and `kanban-export`; queries `kanban-cards`, `kanban-pending`, and `kanban-epic-pending`; pattern `kanban-batch`; and bin `kanban-dash`, and proves source activation plus retained image replay. The historical v24 full repository suite still requires the removed Guild root; that incompatibility is a gate fact and this adapter proof does not claim the full suite is green. Guild, peering operations, and lifecycle resources are absent from the Millstrand surface.
