# Spike: shape of devcycles.loom and its consumers
...
- `devcycles.loom/` — the shared spool repo's shape: `fix` and `land` as exported unbound
  templates plus default-bound registrations, params for repo-varying values, gate commands
  as bindings data, defer points with loom-shipped `:call` targets, the tracker module with
  a real `:removed` branch, and attention rules as an owner contribution.
- `consumers/skein-src.init.clj` — this repo consuming the loom: scripts stay local and ride
...
1. Per-run: the loom binds a defer to an allowlist of several `:call` targets; the driving
...
2. Per-world: a workspace module re-registers the exported template with its own
   `bind-defers` and declared `:overrides`, shadowing the loom's default registration.
