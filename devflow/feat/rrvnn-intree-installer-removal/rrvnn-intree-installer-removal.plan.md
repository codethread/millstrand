...
- **PLAN-Itr-001.CM1:** Deleting exported spool fns is a TEN-000@1 removal authorized by ADR-003.P5; no root-spec change (install! appears in no root spec). No data migration. Canonical world picks up the `.millstrand/init.clj` comment-only change via `runtime/refresh!` — no weaver restart; spool source changes reach the canonical world at the cutover feature's refresh, not here (production already calls only contribute/reconcile, and the C46b dispatch is behavior-compatible on the applied path).
...
- **PLAN-Itr-001.V3:** Grep gate: `rg 'install!' src spools test docs devflow .millstrand --glob '!devflow/archive/**'` output equals the recorded allowlist (kanban_tracker.clj, config_ops_test.clj staged sibling call, history-describing prose).
...
- **PLAN-Itr-001.R4:** The parity test reads `.millstrand/init.clj` from the repo checkout; keep it read-only and path-derived from the repo root so disposable-workspace runs stay hermetic.
