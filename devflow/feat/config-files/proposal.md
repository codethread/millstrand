...
**Document ID:** `PROP-Cff-001` **Status:** Approved **Approved:** 2026-09-23 **Related RFCs:** [Lifecycle authoring history](../../rfcs/2026-07-28-lifecycle-authoring-forms.md); its Proposed text is historical context, not the current lifecycle contract. **Related root specs:** [REPL API](../../specs/repl-api.md), [Weaver Runtime](../../specs/daemon-runtime.md), [Alpha Surface](../../specs/alpha-surface.md) **Brief:** [User decisions](brief.md)

This proposal is approved and frozen. Implementation changes belong in the spec deltas, plan, and code.

## PROP-Cff-001.P1 Problem

Simple spool configuration currently requires Clojure even when each entry is a small map plus prose. Codethread's agent aliases and reviewer lenses are examples: Markdown or YAML files would be easier to edit and group without moving their domain behaviour into a new configuration language.
...
- **PROP-Cff-001.G1:** Configure the common case through Markdown with YAML frontmatter or standalone YAML mappings. Markdown supplies its unchanged body as `:doc`; YAML leaves field names to the author, with `doc` recommended for consistency.
...
- **PROP-Cff-001.NG2:** No TOML or other additional formats, template expressions, environment lookup, includes, typed substitutions, or embedded code execution.
...
The loader returns `{:path "reviewer.md" :config {...}}`: `:config` contains the frontmatter fields plus `:doc` holding the body. The adapter decides whether the filename names the alias, how to use `:doc`, and which other fields it recognises.

For the same data in YAML, use `:format :yaml` and a matching filename glob such as `"*.yaml"`:
...
YAML supplies its mapping directly as `:config`; it adds no `:doc` automatically. Authors may use another key such as `instructions`. Using `doc` is a convention for adapters shared with Markdown, not a required field.
