...
- Support Markdown with YAML frontmatter first. Preserve body text and parse frontmatter as data.
...
- Preserve `{{varname}}` tokens in frontmatter and body. `config/hydrate` is an opt-in literal recursive replacement helper using caller-supplied values, with no environment lookup or evaluation.
...
- Unknown frontmatter fields may be ignored by the domain adapter. Invalid recognised values still fail.
