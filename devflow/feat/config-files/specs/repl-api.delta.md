...
For the first version, `options` is closed to required `:location`, `:format`, and `:close`, plus optional default `:after`. `:format` is either `:markdown` or `:yaml`, selected explicitly rather than inferred from the filename; `:close` is a fully qualified callable symbol. Config-file resources have module lifetime; no `:scope` option ships. The location map is closed to required non-blank strings `:path` and `:filename`, and optional boolean `:recursive?`, default false. The runtime delta defines their filesystem semantics.
...
The open callable takes two arguments: the existing lifecycle context, including explicit `:runtime`, and a document map containing exactly `:path` and `:config`. `:path` is the matched path relative to the declared root, using `/` separators. `:config` is the parsed keyword-keyed configuration map. This envelope keeps file provenance separate from arbitrary user-authored fields, including a domain's own `path` field.

For `:markdown`, merge the frontmatter mapping with `:doc` containing the uninterpreted Markdown body. The body is authoritative: it supplies `:doc` even when empty and replaces any frontmatter `doc` value. For `:yaml`, use the authored mapping as `:config` without inserting, requiring, or renaming a documentation field. `doc` is recommended for consistency with Markdown but any domain field name is valid.

Both formats use the same YAML data mapping: nested maps have keyword keys and sequences become vectors. Strings, numbers, booleans, and nil remain data, with no domain coercion, keyword-valued enum inference, or set conversion.
...
Apart from mapping Markdown body to `:doc`, the loader has no built-in alias identity, registration schema, interpretation of prose, or domain collision policy. Callbacks must finish domain validation before acquisition and must clean up any partial acquisition if they throw before returning a handle. The coordinator does not infer a partial handle from exception data.

### DELTA-Cff-001.CC4 Optional literal hydration

`(config/hydrate bindings configfile)` returns a new document map. `bindings` maps unqualified keywords whose names match `[A-Za-z_][A-Za-z0-9_-]*` to strings. Tokens are exactly `{{` followed by such a name followed by `}}`, with no whitespace; matching is case-sensitive. `:cwd` replaces `{{cwd}}`. Namespaced, blank, or otherwise invalid binding keys fail at this public boundary. Other brace text remains literal. The helper walks all string values in `:config` recursively through maps and vectors, including Markdown's generated `:doc` and any YAML prose field. It leaves map keys, non-string values, and the envelope's provenance `:path` unchanged. A domain's own `:config :path` string is ordinary configuration and can be hydrated.
