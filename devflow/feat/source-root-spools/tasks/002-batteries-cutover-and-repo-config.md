...
- **TASK-Srs-002.MI2:** In `.skein/spools.edn`, seed `skein.spools/batteries {:skein/source-root "spools/batteries"}` and replace the four shipped-spool `../spools/*` local roots for workflow, text-search, chime, and cron with their `:skein/source-root` coordinates (`SPEC-004-D006.C1`, `SPEC-004-D006.C3`, `SPEC-004-D006.C7`).
