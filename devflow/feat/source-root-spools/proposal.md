...
- **PROP-Srs-001.G4:** No `..`-shaped `:local/root` entries naming shipped skein spools remain in this repo's or sibling repos' committed `.millstrand` worlds; the migration itself dogfoods the new kind. `..` roots naming anything else are surfaced with a recommendation, never silently converted.
