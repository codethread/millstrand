...
The probe resolves each copied member basis, then composes one candidate pool classloader from all member bases before starting any member. Refactor the effect-free staging internals used by [`fresh-runtime-probe!`](../../../src/millstrand/core/weaver/runtime.clj#L837-L915) so pooled staging accepts a prebuilt member basis, the shared candidate loader, and an optional old-member baseline. Do not call the existing single-member helper independently: it creates its own loader and requires an admitted baseline. Its isolated contract remains unchanged. Each pooled member uses `:storage :sqlite-memory`, `:publish? false`, and `:probe? true`. Probe mode has no scheduler or event lane, lifecycle apply, process custody, canonical metadata, canonical ready marker, serving endpoint admission, launch custody secret, or Mill admission record. It collects startup and module declarations, stages and validates candidate registries, and stops every probe runtime before a successful replacement cutover.
...
`success: false` uses `stage: "probe/failure"`, retains the same closed fields, and records the structured failure context in the collective diagnostic. Each `members` entry contains the original mapping, candidate identities, `baseline_kind` (`live` or `newcomer`), `status` (`validated` or `failed`), `registry_projection`, `registry_diff` (a redacted added/removed/changed diff for `live`, `null` for `newcomer`), and `member_diagnostic`. The child stops every probe runtime, writes its result, and exits without deleting the probe root. Mill waits for successful child exit, reads and validates the complete result against the manifest, retains the result projection in its restart record, then removes the private probe root and confirms cleanup. Only then may Mill atomically close old admission and record `restarting`. A failed probe or incomplete cleanup leaves old admission open and retains available diagnostics; failed probe children are stopped best-effort. After old-host stop, replacement failure records `failed` and never re-admits the old host.

Mill accepts `success:true` only when the result's probe identity and private paths match the manifest and its member array covers the manifest one-to-one, in manifest order, with no duplicates, omissions, or extras. Each entry must match the original configuration directory and candidate identities, have the expected live/newcomer baseline kind and diff semantics, carry a valid complete candidate registry projection, and have `status:"validated"`. Any mismatch or failed member is probe failure regardless of the top-level success flag: retain old admission and available diagnostics, with no successful cleanup or cutover. Tests must reject success results with an omitted, duplicate, unexpected, mismatched, or failed member.
