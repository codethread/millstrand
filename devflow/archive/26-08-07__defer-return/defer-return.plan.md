...
- `spools/workflow` after the cutover: 2041 lines in the public story file plus 2827 across ten `internal/*` concerns, 4868 total; less ~154 of per-file ns overhead, the approximate single-file fold is ~4714 lines. That is roughly 9.4x the ~500-line tipping point — the public story file alone is 4x it before a single concern merges in — so the per-concern split is the deliverable and the fold-back branch does not apply. The cutover made the module smaller (deleting dispatch, the old transfer, and their vocabulary net ~350 lines of shipped source), nowhere near enough to change the decision.
- The story run recorded the split-first review on gate `l0lio` (run `7e0la`, sol-med): concern boundaries sound, public `compile`/`defer!`/`run-defer!` retain visible orchestration, dependency graph acyclic, no test depends on an internal namespace. Its one blocking finding — the CLI suites still driving the removed verbs — is `TASK-Dfr-002.DW1` verbatim and is adjudicated to that task rather than fixed here.

### PLAN-Dfr-001.DN5 Task 2, worker and docs cutover — 2026-07-26

- The generic worker now has one runtime-selection verb, `workflow defer`, backed by `run-defer!` and `::defer-request`. Its declared arg-spec is the help contract, so removing the other verbs from that map removes them from parsing and help together; CLI tests also pass the old verb names to the parser and assert loud refusal.
