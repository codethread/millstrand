...
- **TASK-Dyc-001.MI2:** Steps may `:depends-on` a dispatch id. Scope `validate-defer-topology!` to defer steps only so it no longer sees dispatch steps; a step depending on a *defer* must still fail exactly as today.
...
- **TASK-Dyc-001.MI7:** `require-no-defers!` branched per CC12: a dispatch target may not declare a *terminal defer*, but may declare a dispatch of its own.
