...
- **DELTA-Sad-002.CC7:** A fresh source-mode generation can load annotated Kanban v26 at peeled SHA `cd6eab928408faf7101af612c2e199796852d669` against the new Millstrand macros, then activate the published `ct.spools.kanban` module: ops `kanban` and `kanban-export`; queries `kanban-cards`, `kanban-pending`, and `kanban-epic-pending`; pattern `kanban-batch`; and bin `kanban-dash`. Guild, peering operations, and lifecycle resources are absent from the published module surface. A disposable fresh-generation acceptance proof is required before restart, and the final external spool gate runs the published v26 repository suite. Its retained declaration record follows CC4 for image replay. No compatibility bridge is part of this contract.
