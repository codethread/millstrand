...
- `.millstrand/spools.edn` and `.millstrand/init.clj`
...
- **TASK-Dsp-009.MI1:** Bump `.millstrand/spools.edn` sibling pins to the Phase B released markers (Task 8) and drop the remaining sibling-backed `:contribute`/`:reconcile` triples from `.millstrand/init.clj`.
...
- **TASK-Dsp-009.DW1:** The epic DONE-WHEN greps hold: `git grep '(def module'` in `spools/` is empty and `.millstrand/init.clj` carries no `:contribute`/`:reconcile`.
