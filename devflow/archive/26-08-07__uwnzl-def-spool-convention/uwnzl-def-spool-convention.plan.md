...
| PLAN-Dsp-001.AA4 | In-tree `spools/*`, `.millstrand/*.clj`, `.millstrand/init.clj` | `def module` → `def spool` (drop `:ns`), file-module `spool` vars, in-tree init triples dropped, parity test narrowed |
...
Outcome: the transitional window closes. Bump sibling pins, drop the remaining sibling-backed triples from `.millstrand/init.clj`, sweep the remaining core tests and fixtures that generate explicit entry-point keys, remove `:contribute`/`:reconcile` from `::module-opts`, delete the narrowed parity test, land the Phase C spec delta with the promoted C19 exception, refresh the canonical world, and verify live — the close-out shape feature `rtnfv` used for epic `waq0l`. The epic DONE-WHEN grep gates (`git grep '(def module'` in `spools/` empty; no `:contribute`/`:reconcile` in `.millstrand/init.clj`) hold from here. Depends on Phase B released.
...
- The `.millstrand/init.clj` header no longer claims every module resolves entry points from a `spool` var; `attention.clj`, `nvd_scan.clj`, `config.clj`, `analytics.clj` and the macro namespaces contribute only collected authoring forms and declare none.
