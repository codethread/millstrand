...
const freshRuntimeProbeExpression = `(require 'clojure.data.json 'millstrand.core.db 'millstrand.core.weaver.runtime) (let [wire-baseline (clojure.data.json/read-str (slurp (System/getenv "MILLSTRAND_PROBE_BASELINE_PATH"))) baseline {:status (keyword (get wire-baseline "status")) :projection (get wire-baseline "projection")} result (millstrand.core.weaver.runtime/fresh-runtime-probe! {:config-dir (System/getenv "MILLSTRAND_PROBE_CONFIG") :state-dir (System/getenv "MILLSTRAND_PROBE_STATE") :data-dir (System/getenv "MILLSTRAND_PROBE_DATA")} {:old-generation-baseline baseline}) stage->wire (fn [value] (if (keyword? value) (subs (str value) 1) value)) wire (-> (select-keys result [:success :stage :probe/workspace :source/workspace :completed :diagnostics :log]) (update :stage stage->wire) (update :completed #(mapv stage->wire %)))] (spit (System/getenv "MILLSTRAND_PROBE_RESULT") (clojure.data.json/write-str wire :key-fn millstrand.core.db/json-key)))`
