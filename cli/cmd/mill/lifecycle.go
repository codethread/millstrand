...
func validateLaunchSource(label, source string) (string, error) {
	...
	if _, err := config.ValidateSourceVersion(resolved, config.Version); err != nil {
		return "", fmt.Errorf("%s release identity is invalid: %w", label, err)
	}
	return resolved, nil
}
...
func validateMetadata(world config.World, m client.Metadata) string {
	if m.ProtocolVersion != client.ProtocolVersion {
		return fmt.Sprintf("weaver protocol mismatch: got v%d, mill supports v%d", m.ProtocolVersion, client.ProtocolVersion)
	}
	if m.Version == "" || m.PID == 0 || m.DaemonID == "" || m.ConfigDir == "" || m.StateDir == "" || m.DataDir == "" || strings.TrimSpace(m.Name) == "" || m.SocketPath == "" || m.StartedAt == "" || m.NREPL.Host == "" || m.NREPL.Port == 0 || !validBasisFingerprint(m.BasisFingerprint) {
