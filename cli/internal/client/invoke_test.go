...
func TestRelaySingleSuccessPrintsResult(t *testing.T) {
	out, er, code := relay(t, `{"protocol_version":3,"request_id":"r1","ok":true,"result":{"id":"task-1"},"error":null}`+"\n")
	if code != 0 {
...
func TestRelaySingleSuccessDoesNotHTMLEscapeResult(t *testing.T) {
	out, er, code := relay(t, `{"protocol_version":3,"request_id":"r1","ok":true,"result":{"usage":"strand kanban <id>"},"error":null}`+"\n")
	if code != 0 {
		t.Fatalf("expected exit 0, got %d (stderr=%q)", code, er)
	}
...
func TestRelaySingleVerbatimPrintsRawText(t *testing.T) {
	...
	frame := `{"protocol_version":3,"request_id":"r1","ok":true,"result":"RENDERED add: usage <id>","error":null,"verbatim":true}` + "\n"
...
func TestRelaySingleVerbatimPreservesExistingTrailingNewline(t *testing.T) {
	...
	frame := `{"protocol_version":3,"request_id":"r1","ok":true,"result":"line one\nline two\n","error":null,"verbatim":true}` + "\n"
...
func TestRelaySingleNonVerbatimStringStaysJSON(t *testing.T) {
	...
	out, _, code := relay(t, `{"protocol_version":3,"request_id":"r1","ok":true,"result":"plain","error":null}`+"\n")
	if code != 0 {
		t.Fatalf("expected exit 0, got %d", code)
	}
...
func TestRelaySingleErrorGoesToStderrNonZero(t *testing.T) {
	frame := `{"protocol_version":3,"request_id":"r1","ok":false,"result":null,"error":{"type":"domain","code":"op/not-found","message":"Operation not found","details":{"available":["add","list"]}}}` + "\n"
	var out, er bytes.Buffer
...
func TestRelayStreamRelaysLinesThenExitsBySuccess(t *testing.T) {
	frames := strings.Join([]string{
		`{"protocol_version":3,"request_id":"r1","stream":true}`,
		`{"i":0}`,
		`{"i":1}`,
		`{"protocol_version":3,"request_id":"r1","done":true,"success":true,"result":{"emitted":2}}`,
	}, "\n") + "\n"
...
func TestRelayStreamErrorTerminatorNonZero(t *testing.T) {
	frames := strings.Join([]string{
		`{"protocol_version":3,"request_id":"r1","stream":true}`,
		`{"i":0}`,
		`{"protocol_version":3,"request_id":"r1","done":true,"success":false,"error":{"type":"domain","code":"stream/failed","message":"stream failed","details":{}}}`,
	}, "\n") + "\n"
	out, er, code := relay(t, frames)
	if code == 0 {
...
func TestRelayStreamOverSocketFlushesIncrementally(t *testing.T) {
	...
	go func() {
		...
		lines := []string{
			`{"protocol_version":3,"request_id":"r1","stream":true}`,
			`{"i":0}`,
			`{"protocol_version":3,"request_id":"r1","done":true,"success":true,"result":null}`,
		}
		for _, l := range lines {
			_, _ = w.WriteString(l)
			_, _ = w.WriteString("\n")
...
func TestRelayStreamTruncatedFailsNonZero(t *testing.T) {
	...
	frames := `{"protocol_version":3,"request_id":"r1","stream":true}` + "\n" + `{"i":0}` + "\n"
