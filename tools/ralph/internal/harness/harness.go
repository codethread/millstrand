...
func Prompt(user, epicID, epicTitle string, iteration int) string {
	...
	b.WriteString(") ---\n")
	fmt.Fprintf(&b, "You are one iteration of \"ralph\", a headless loop driving epic %s (%q).\n", epicID, epicTitle)
	...
	b.WriteString("- Claim and complete exactly ONE feature card (claim, work its tasks, land,\n")
	b.WriteString("  finish the card), then end your reply.\n")
